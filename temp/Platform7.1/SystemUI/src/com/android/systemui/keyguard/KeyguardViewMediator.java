...
import com.android.keyguard.TouchProtectScreen;
import com.android.keyguard.ViewMediatorCallback;
import com.android.keyguard.YLClassProxy;
...
import com.android.systemui.statusbar.phone.StatusBarThemeViewManager;
...
public class KeyguardViewMediator extends SystemUI {
    ...
    //add by mengludong 2015.11.17
    private static final int SET_COVERED = 75;
    private static final int SET_LOCK_THEME = 76;
    //add end
    ...
    KeyguardUpdateMonitorCallback mUpdateCallback = new KeyguardUpdateMonitorCallback() {
		...
		/**
         * Update hall sensor state to show or hide cover view
         * @see com.android.keyguard.KeyguardUpdateMonitorCallback#onHallStateChanged(int)
         * @add by mengludong 2015.11.17
         */
		@Override
		public void onHallStateChanged(int state) {
			if (DEBUG) Log.v(TAG, "onHallStateChanged() state = " + state);
			setCovered(state);
		}

		/**
		 * Update antitheft state to show or hide antitheft view
		 * @see com.android.keyguard.KeyguardUpdateMonitorCallback#onAntitheftStateChanged(boolean)
		 * @add by mengludong 2015.11.17
		 */
		@Override
		public void onAntitheftStateChanged(boolean showing) {
			if (showing) {
				doKeyguardLocked(null);
			}else {
				resetStateLocked();
		...
		/**
		 * Update lock theme
		 * @see com.android.keyguard.KeyguardUpdateMonitorCallback#onCoolShowThemeChanged(java.lang.String)
		 * @add by mengludong 2015.12.03
		 */
		@Override
		public void onCoolShowThemeChanged(String path, boolean reload, boolean isVilfe) {
			if (DEBUG) Log.v(TAG, "onCoolShowThemeChanged()");
			setLockTheme(path, reload, isVilfe);
		}
    };
    ...
    ViewMediatorCallback mViewMediatorCallback = new ViewMediatorCallback() {
        ...
        @Override
        public void resetKeyguard(boolean relock) {
            if (relock && !mShowing) {
                doKeyguardLocked(null);
            } else {
                resetStateLocked();
	...
	private boolean mCrashedWhenLoadingTheme;
    ...
    private void setupLocked() {
        ...
        mCrashedWhenLoadingTheme = getDonntLockFlag();
		if (mCrashedWhenLoadingTheme) {
			StatusBarThemeViewManager.setDonntLockFlag(mContext, false);
		}
        // Assume keyguard is showing (unless it's disabled) until we know for sure...
        setShowingLocked(!shouldWaitForProvisioning() && !isLockScreenDisabled());
	...
	private boolean isLockScreenDisabled() {
		return mLockPatternUtils.isLockScreenDisabled(KeyguardUpdateMonitor.getCurrentUser())
				|| mCrashedWhenLoadingTheme;
	}

	private boolean getDonntLockFlag() {
        SharedPreferences preferences = mContext.getSharedPreferences("KeyguardPreferences", Context.MODE_PRIVATE);
        return preferences.getBoolean("donnt_lock_when_restart", false);
    }
    ...
    public void onSystemReady() {
        ...
        synchronized (this) {
            ...
            //add by menguldong, show antitheft pattern when the device first boots.
            sendAntitheftStateChangeIfNeeded();
            //add end
        }
        ...
        mCrashedWhenLoadingTheme = false;
    }
...
//        Intent intent = new Intent("com.android.systemui.lock");
//    	mContext.sendBroadcast(intent);
        synchronized (this) {
            ...
            if (mPendingLock && why != WindowManagerPolicy.OFF_BECAUSE_OF_TIMEOUT) {
                playSounds(true);
            }
        }
    ...
    private long getLockTimeout(int userId) {
        ...
        //#ifdef _SS_SPACES_SERVICE
                // stock AOSP always gets the LOCK_SCREEN_LOCK_AFTER_TIMEOUT for the owner space.
                // That's a bug. This gets it for the correct space.
		final long lockAfterTimeout = Settings.Secure.getIntForUser(cr, Settings.Secure.LOCK_SCREEN_LOCK_AFTER_TIMEOUT,
				KEYGUARD_LOCK_AFTER_DELAY_DEFAULT, KeyguardUpdateMonitor.getCurrentUser());
        ...
        if (policyTimeout <= 0) {
            ...
            // stock AOSP always gets the SCREEN_OFF_TIMEOUT for the owner space.
            // That's a bug. This gets it for the correct space.
            long displayTimeout = Settings.System.getIntForUser(cr, SCREEN_OFF_TIMEOUT,
                    KEYGUARD_DISPLAY_TIMEOUT_DELAY_DEFAULT, KeyguardUpdateMonitor.getCurrentUser());
    ...
    private void maybeSendUserPresentBroadcast() {
        if (mSystemReady && isLockScreenDisabled()) {
    ...
    private void handleSetOccluded(boolean isOccluded) {
        synchronized (KeyguardViewMediator.this) {
            ...
            if (mOccluded != isOccluded) {
                ...
                mUpdateMonitor.setOccluded(isOccluded);
    ...
    private void doKeyguardLocked(Bundle options) {
        ...
        if (!UserManager.isSplitSystemUser()
			...
			if (isLockScreenDisabled() && !lockedOrMissing) {
    ...
    public boolean isSecure() {
        return mLockPatternUtils.isSecure(KeyguardUpdateMonitor.getCurrentUser())
            || KeyguardUpdateMonitor.getInstance(mContext).isSimPinSecure() || KeyguardUpdateMonitor.getInstance(mContext).isAntitheftPattern();
    }
    ...
    public void keyguardDone(boolean authenticated) {
        ...
        synchronized (KeyguardViewMediator.this) {
            ...
            mHiding = true;
            Log.d("fingerWakeupSlowLog:", "mShowing = " + mShowing + ", mOccluded = " + mOccluded + " hanim="+mHideAnimationRun);
            if (mShowing && !mOccluded) {
	...
	private Handler mHandler = new Handler(Looper.myLooper(), null, true /* async */) {
		...
		public void handleMessage(Message msg) {
			...
			case ON_ACTIVITY_DRAWN:
				...
				break;
			// 2015.12.03 add by mengludong
			case SET_COVERED:
				handleSetCovered(msg.arg1 == 1);
				break;
			case SET_LOCK_THEME:
				handleSetLockTheme(msg.arg1 == 1, msg.arg2 == 1, (String) msg.obj);
				break;
			// add end
			}
		}
    ...
    private void handleKeyguardDone(boolean authenticated) {
        ...
        Log.d("fingerWakeupSlowLog:", "handleKeyguardDone()");
        handleHide();
    }
                ...
                for (int profileId : um.getProfileIdsWithDisabled(currentUser.getIdentifier())) {
                	try{
                      mContext.sendBroadcastAsUser(USER_PRESENT_INTENT, UserHandle.of(profileId));
						          ...
						          Log.e(TAG, "sendUserPresentBroadcast error: " + e);
					          }
                }
    ...
    private void handleHide() {
        synchronized (KeyguardViewMediator.this) {
            ...
            mHiding = true;
            Log.d("fingerWakeupSlowLog:", "mShowing = " + mShowing + ", mOccluded = " + mOccluded);
            if (mShowing && !mOccluded) {
                if (!mHideAnimationRun) {
                    mStatusBarKeyguardViewManager.startPreHideAnimation(mKeyguardGoingAwayRunnable);
                } else {
    ...
    private void handleStartKeyguardExitAnimation(long startTime, long fadeoutDuration) {
    	 Log.d("fingerWakeupSlowLog:", "handleStartKeyguardExitAnimation is come");
        synchronized (KeyguardViewMediator.this) {
            ...
            Log.d("fingerWakeupSlowLog:", "mWakeAndUnlocking = " + mWakeAndUnlocking + ", mDrawnCallback = " +(mDrawnCallback != null));
            ...
            setShowingLocked(false);
            Log.d("fingerWakeupSlowLog:", "hide() before");
            mStatusBarKeyguardViewManager.hide(startTime, fadeoutDuration);
            Log.d("fingerWakeupSlowLog:", "hide() over");
            resetKeyguardDonePendingLocked();
            mHideAnimationRun = false;
            updateActivityLockScreenState();
            adjustStatusBarLocked();
            sendUserPresentBroadcast();
            mUpdateMonitor.makeSureStopFingerListenning();
            Log.d("fingerWakeupSlowLog:", "hide() end");
        }
    }
    ...
    private void notifyDrawn(final IKeyguardDrawnCallback callback) {
        try {
        	Log.d("fingerWakeupSlowLog:", "call IKeyguardDrawnCallback.onDrawn()");
            callback.onDrawn();
        } catch (RemoteException e) {
    ...
    private void setShowingLocked(boolean showing) {
        if (showing != mShowing) {
            ...
            updateInputRestrictedLocked();
            mTrustManager.reportKeyguardShowingChanged();
            mUpdateMonitor.sendKeyguardShowingChanged(showing);
        }
    }
    ...
    /**
	 *  Send antitheft state change if needed
	 * @add by mengludong 2015.11.18
	 */
	private void sendAntitheftStateChangeIfNeeded() {
		SharedPreferences preferences = mContext.getSharedPreferences("KeyguardPreferences", Context.MODE_PRIVATE);
		long deviceRunTime = SystemClock.elapsedRealtime();
		if (preferences.getBoolean("ignore_antitheft_state", false) || deviceRunTime > 600000) {
			Log.d(TAG, "sendAntitheftStateChangeIfNeeded() ignore antitheft state because systemui crashed.");
			Editor editor = preferences.edit();
			editor.putBoolean("ignore_antitheft_state", false);
			editor.commit();
			return;
		}
		int showType = showKeyguardType();
		if (showType == 1) {
			mUpdateMonitor.sendAntitheftStateChanged(true);
		} else if (showType == 2) {
			//mUpdateMonitor.sendRebootPassStateChanged(true);
		}
		Log.d(TAG, "sendAntitheftStateChangeIfNeeded() showType = " + showType + ", deviceRunTime = " + deviceRunTime);
	}
	
	/**
	 *  Check should we display antitheft pattern
	 * @return show type
	 * @add by mengludong 2015.11.18
	 */
	private int showKeyguardType() {
		try {
			Object systemInterface = YLClassProxy.SystemInterfaceFactory_getSysteminterface();
			int showType = YLClassProxy.ISystemInterface_getShowKeyguardType(systemInterface);
			return showType;
		} catch (Exception e) {
			Log.w(TAG, "showKeyguardType() is exception");
		}
		return 0;
	}
	
	/**
	 * Send lock theme change msg
	 * @param path
	 * @add by mengludong 2015.12.03
	 */
	protected void setLockTheme(String path, boolean reload, boolean isVilfe) {
		mHandler.removeMessages(SET_LOCK_THEME);
		Message msg = mHandler.obtainMessage(SET_LOCK_THEME, reload ? 1 : 0, isVilfe ? 1 : 0, path);
		mHandler.sendMessage(msg);
	}
	
	/**
	 * Handles SET_LOCK_THEME message sent by setLockTheme()
	 * @param path
	 * @add by mengludong 2015.12.03
	 */
	protected void handleSetLockTheme(boolean reload, boolean isVlife, String path) {
		if (DEBUG) Log.d(TAG, "handleSetLockTheme()");
		synchronized (KeyguardViewMediator.this) {
			mStatusBarKeyguardViewManager.setLockTheme(path, reload, isVlife);
			/*if (!mShowing) {
				doKeyguardLocked(null);
			}*/
		}
	...
	/**
	 * Notify us when the cell phone is covered by holster
	 * @add by mengludong 2015.11.17
	 * @param isCovered
	 */
	private void setCovered(int state) {
		if (DEBUG)
			Log.d(TAG, "setCovered " + state);
		mHandler.removeMessages(SET_COVERED);
		Message msg = mHandler.obtainMessage(SET_COVERED, state, 0);
		mHandler.sendMessage(msg);
	}
	
	/**
	 * Handles SET_COVERED message sent by setCovered()
	 * @param isCovered
	 * @add by mengludong 2015.11.17
	 */
	private void handleSetCovered(boolean isCovered) {
		if (DEBUG) Log.d(TAG, "handleSetCovered " + isCovered);
		synchronized (KeyguardViewMediator.this) {
			if (mCovered != isCovered) {
				mCovered = isCovered;
				mStatusBarKeyguardViewManager.setCovered(mCovered);
				if (mCovered) {
					if (!mUpdateMonitor.isAntitheftPattern() && !mUpdateMonitor.isSimPinSecure()) {
						int trunOffScreen = Settings.System.getInt(mContext.getContentResolver(),
								"judge_hall_sensor_screen_off", 0);
						if (trunOffScreen != 0) {

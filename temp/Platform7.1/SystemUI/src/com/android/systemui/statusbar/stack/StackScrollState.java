...
public class StackScrollState {
    ...
    public void apply() {
        ...
        for (int i = 0; i < numChildren; i++) {
            ExpandableView child = (ExpandableView) mHostView.getChildAt(i);
            ExpandableView childReverse = (ExpandableView) mHostView.getChildAt(numChildren-i-1);
            /*if(childReverse.getVisibility() == View.VISIBLE && 
            		childReverse instanceof ActivatableNotificationView){
            	ActivatableNotificationView v = (ActivatableNotificationView)childReverse;
            	View topLine = v.getTopLine();
            	View bottomLine = v.getBottomLine();
            	if(topLine != null && bottomLine != null){
            		if(lastVisibleCount == 0){//lastChild
            			topLine.setVisibility(View.VISIBLE);
            			bottomLine.setVisibility(View.VISIBLE);
            			lastChildIndex = numChildren-i-1;
            		}else{
            			topLine.setVisibility(View.VISIBLE);
...
//                dismissView.performVisibilityAnimation(visible && !dismissView.willBeGone());
        ...
        /*for (int i = 0; i < numChildren; i++) {
            ...
            if(child.getVisibility() == View.VISIBLE && 
            	...
            	if(firstVisibleCount == 0){
            		firstChildIndex = i;
            		if(((NotificationStackScrollLayout)mHostView).mPhoneStatusBar.getBarState()!=StatusBarState.KEYGUARD){
            		ActivatableNotificationView v = (ActivatableNotificationView)child;
                	View topLine = v.getTopLine();
                	View bottomLine = v.getBottomLine();
        ...
        /*for (int i = 0; i < numChildren; i++) {//===modify by ty for Notification
            ExpandableView child = (ExpandableView) mHostView.getChildAt(i);
            if(child.getVisibility() == View.VISIBLE && 
            		child instanceof ActivatableNotificationView){
            	if(firstVisibleCount == 0){
                	...
                	View topLine = v.getTopLine();
                	View bottomLine = v.getBottomLine();
            		if(((NotificationStackScrollLayout)mHostView).mPhoneStatusBar.getBarState()!=StatusBarState.SHADE){
            		    topLine.setVisibility(View.VISIBLE);
        			    bottomLine.setVisibility(View.GONE);
            		}else{
            			topLine.setVisibility(View.GONE);
    ...
    public void applyViewState(View view, ViewState state) {
        ...
        if (yTranslation != newYTranslation) {
        	if(view instanceof EmptyShadeView){
        		view.setTranslationY(Utilities.dipToPixel(view.getContext(), 200+17)+newYTranslation);
        	}else{

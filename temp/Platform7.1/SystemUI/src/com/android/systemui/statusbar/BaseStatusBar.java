...
public abstract class BaseStatusBar extends SystemUI implements
            ...
            if (sbn != null) {
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        Entry oldEntry = mNotificationData.get(key);
                        
                        StatusBarNotification notification = sbn;
						...
						if (oldEntry != null
								&& oldEntry.notification != null
								&& oldEntry.notification.getNotification() != null) {
                        	textChange =  !TextUtils.equals(n.tickerText,
                    				oldEntry.notification.getNotification().tickerText);
                        }
                        boolean ongoing = (n.flags & Notification.FLAG_ONGOING_EVENT) != 0;
                        boolean updateTicker = !TextUtils.isEmpty(n.tickerText) && (n.tickerText != null);
                        if (DEBUG) Log.d(TAG, "updateTicker=" + updateTicker + " ongoing="+ongoing+" text="+n.tickerText);
                        
                        boolean isForCurrentUser = isNotificationForCurrentProfiles(notification);
                        if (DEBUG) Log.d(TAG, "notification is " + (isForCurrentUser ? "" : "not ") + "for you");
                        
                        if (textChange && updateTicker && isForCurrentUser) {
                            haltTicker();
    ...
    public void start() {
        ...
        mCurrentUserId = CurrentUserTracker.getCurrentUserIdAquire();
    ...
    protected void performRemoveNotification(StatusBarNotification n, boolean removeView) {
        ...
        try {
        	LogHelper.sd(TAG, "700007_z onNotificationClear pkg="+pkg+" tag="+tag+" id="+id+" userId="+userId);
            mBarService.onNotificationClear(pkg, tag, id, userId);
            LogHelper.sd(TAG, "700007_0 onNotificationClear");
            if (FORCE_REMOTE_INPUT_HISTORY
                    && mKeysKeptForRemoteInput.contains(n.getKey())) {
                mKeysKeptForRemoteInput.remove(n.getKey());
                removeView = true;
                LogHelper.sd(TAG, "700007_1 FORCE_REMOTE_INPUT_HISTORY");
            }
            if (mRemoteInputEntriesToRemoveOnCollapse.remove(mNotificationData.get(n.getKey()))) {
                removeView = true;
                LogHelper.sd(TAG, "700007_2 mRemoteInputEntriesToRemoveOnCollapse");
            }
            if (removeView) {
            	LogHelper.sd(TAG, "700007_3 mRemoteInputEntriesToRemoveOnCollapse");
                removeNotification(n.getKey(), null);
            }

        } catch (RemoteException ex) {
            // system process is dead if we're here.
        	 LogHelper.sd(TAG, "700007_4 mRemoteInputEntriesToRemoveOnCollapse");
        	ex.printStackTrace();
        }
    }
    ...
    public boolean inKeyguardRestrictedInputMode() {
		return mStatusBarKeyguardViewManager != null ? mStatusBarKeyguardViewManager.isInputRestricted() : false;
    }
        ...
        return new SwipeHelper.LongPressListener() {
            ...
            public boolean onLongPress(View v, final int x, final int y) {
            	if(true){
                	...
                	intent.setClass(mContext, PackageSettingsActivity.class);
                	if (v instanceof ExpandableNotificationRow) {
                		ExpandableNotificationRow row = (ExpandableNotificationRow) v;
                    	final StatusBarNotification sbn = row.getStatusBarNotification();
                    	if(sbn != null){
                    		PackageManager pmUser = getPackageManagerForUser(
                                    mContext, sbn.getUser().getIdentifier());
    ...
    public abstract int getStatusBarIconColor();
    public abstract void setNotificationTemplateIconStyle(View contentView,StatusBarNotification sbn);
    /**
     * 1:Local 2:Big 3:Public 4:Heads 
     * @param contentView 
     * @param sbn
     * @param type
     */
    
    private void setCustomNotificationStyle(View contentView,int type){
    	final Resources res = mContext.getResources();//
    		...
    		if (view instanceof TextView) {
    			...
    			if (viewID > 0) {
        			...
        			if (viewName.equals("title")) {
        				((TextView) view).setTextColor(res.getColor(R.color.notify_title_default_color, null));
					}else if (viewName.equals("text")) {
						((TextView) view).setTextColor(res.getColor(R.color.notify_default_color, null));
						if (type == 1) {
				...
				setCustomNotificationStyle(view,type);
//				view.setBackgroundColor(res.getColor(R.color.notify_background_transparent, null));
			}
		}
		 	
    }
    
	public void setNotificationStyle(View contentView,
			StatusBarNotification sbn, int type) {

		if (contentView == null) {
			if (Utils.isNeedLog) {
				Log.d(TAG, "contentView == null type=" + type);
			}
			return;
		}
		Button notificationAction = null;
		TextView notificationTitle = null;
	    TextView notificationText = null;
	    TextView notificationText2 = null;
	    TextView notificationInfo = null;
	    TextView notificationBigText = null;
	    TextView notificationInboxText0 = null;
	    TextView notificationInboxText1 = null;
	    TextView notificationInboxText2 = null;
	    TextView notificationInboxText3 = null;
	    TextView notificationInboxText4 = null;
	    TextView notificationInboxText5 = null;
	    TextView notificationInboxText6 = null;
	    ImageView notificationActionDivider = null;
	    LinearLayout notificationActions = null;
		...
		// return;
		// }
		contentView.setBackgroundColor(res.getColor(R.color.notify_content_background_color, null));
		setNotificationTemplateIconStyle(contentView, sbn);
		try {
         	ImageView v = null;
		...
		int errorFlag = 0;
		try {
        	if (contentView.findViewById(com.android.internal.R.id.title) instanceof TextView) 
        		notificationTitle = (TextView) contentView.findViewById(com.android.internal.R.id.title);
        	 if (notificationTitle != null){
        		notificationTitle.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 16);
             	notificationTitle.setTextColor(res.getColor(R.color.notify_title_default_color, null));
             }else {
            	 errorFlag++;
//            	 contentView.setBackgroundColor(res.getColor(R.color.notify_background_transparent, null));
            	 contentView.setBackgroundColor(res.getColor(R.color.notify_content_background_color, null));
            	 setCustomNotificationStyle(contentView,type);//set custom notification text color
             }
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "title Exception:"+e);
			errorFlag++;
		}
		...
		if(errorFlag == 1){
			try {
				contentView.setBackgroundColor(res.getColor(R.color.notify_content_background_color, null));
				setCustomNotificationStyle(contentView,type);
			} catch (Exception e2) {
        ...
        try {
        	if (contentView.findViewById(com.android.internal.R.id.text) instanceof TextView) 
        		notificationText = (TextView) contentView.findViewById(com.android.internal.R.id.text);
        	 if (notificationText != null){
             	notificationText.setTextColor(res.getColor(R.color.notify_default_color, null));
             	notificationText.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 12);
             	if (type == 1) {
             		notificationText.setSingleLine(true);
                 	notificationText.setEllipsize(TextUtils.TruncateAt.valueOf("END"));
     			}
             }
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "text Exception:"+e);
		}
        ...
        try {
        	...
        	if (notificationTime != null){
        		notificationTime.setTextColor(res.getColor(R.color.notify_default_color, null));
        		notificationTime.setTextSize(TypedValue.COMPLEX_UNIT_DIP, 10);
        	}
		} catch (Exception e) {
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.text2) instanceof TextView) 
        		notificationText2 = (TextView) contentView.findViewById(com.android.internal.R.id.text2);
        	if (notificationText2 != null) {
            	notificationText2.setTextColor(res.getColor(R.color.notify_default_color, null));
            }
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "text2 Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.info) instanceof TextView) 
        		notificationInfo = (TextView) contentView.findViewById(com.android.internal.R.id.info);
        	 if (notificationInfo != null) {
             	notificationInfo.setTextColor(res.getColor(R.color.notify_default_color, null));
             }
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "info Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.big_text) instanceof TextView) 
        		notificationBigText = (TextView) contentView.findViewById(com.android.internal.R.id.big_text);
        	if (notificationBigText != null) 
            	notificationBigText.setTextColor(res.getColor(R.color.notify_default_color, null));
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e("zxh", "big_text Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.inbox_text0) instanceof TextView) 
        		notificationInboxText0 = (TextView) contentView.findViewById(com.android.internal.R.id.inbox_text0);
        	if (notificationInboxText0 != null ) 
            	notificationInboxText0.setTextColor(res.getColor(R.color.notify_default_color, null));
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "inbox_text0 Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.inbox_text1) instanceof TextView) 
        		notificationInboxText1 = (TextView) contentView.findViewById(com.android.internal.R.id.inbox_text1);
        	if (notificationInboxText1 != null ) 
				notificationInboxText1.setTextColor(res.getColor(R.color.notify_default_color, null));
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "inbox_text1 Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.inbox_text2) instanceof TextView) 
        		notificationInboxText2 = (TextView) contentView.findViewById(com.android.internal.R.id.inbox_text2);
        	if (notificationInboxText2 != null ) 
            	notificationInboxText2.setTextColor(res.getColor(R.color.notify_default_color, null));
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "inbox_text2 Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.inbox_text3) instanceof TextView) 
        		notificationInboxText3 = (TextView) contentView.findViewById(com.android.internal.R.id.inbox_text3);
        	if (notificationInboxText3 != null ) 
            	notificationInboxText3.setTextColor(res.getColor(R.color.notify_default_color, null));
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "inbox_text3 Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.inbox_text4) instanceof TextView) 
        		notificationInboxText4 = (TextView) contentView.findViewById(com.android.internal.R.id.inbox_text4);
        	if (notificationInboxText4 != null ) 
            	notificationInboxText4.setTextColor(res.getColor(R.color.notify_default_color, null));
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "inbox_text4 Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.inbox_text5) instanceof TextView) 
        		notificationInboxText5 = (TextView) contentView.findViewById(com.android.internal.R.id.inbox_text5);
        	if (notificationInboxText5 != null ) 
            	notificationInboxText5.setTextColor(res.getColor(R.color.notify_default_color, null));
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "inbox_text5 Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.inbox_text6) instanceof TextView) 
        		notificationInboxText6 = (TextView) contentView.findViewById(com.android.internal.R.id.inbox_text6);
        	if (notificationInboxText6 != null ) 
            	notificationInboxText6.setTextColor(res.getColor(R.color.notify_default_color, null));
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "inbox_text6 Exception:"+e);
		}
        
        try {
        	if (contentView.findViewById(com.android.internal.R.id.app_name_text) instanceof TextView) 
        		notificationText = (TextView) contentView.findViewById(com.android.internal.R.id.app_name_text);
        	 if (notificationText != null){//secondary_text_default_material_light
        		 //res.getColor(com.android.internal.R.color.datepicker_default_normal_text_color_material_light, null)
//             	notificationText.setTextColor(Color.parseColor("#ff999999"));
             	notificationText.setTextColor(res.getColor(R.color.notify_default_color, null));
             }
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "text Exception:"+e);
        ...
        /*try {
        	if (contentView.findViewById(com.android.internal.R.id.action_divider) instanceof ImageView) 
        		notificationActionDivider = (ImageView) contentView.findViewById(com.android.internal.R.id.action_divider);
        	 if (notificationActionDivider != null){
        		//set line high
        		LayoutParams params = notificationActionDivider.getLayoutParams();
        		params.height = 1;
        		notificationActionDivider.setLayoutParams(params);
        		
        		notificationActionDivider.setBackgroundColor(res.getColor(R.color.notify_mid_line_default_color, null));
        	 }
             	
		} catch (Exception e) {
			if (Utils.isNeedLog) Log.e(TAG, "action_divider Exception:"+e);
		}*/
        
        try {
        	/*if (contentView.findViewById(com.android.internal.R.id.big_picture) instanceof ImageView) {
        		if (contentView.findViewById(com.android.internal.R.id.actions) instanceof LinearLayout) {
        		notificationActions	= (LinearLayout) contentView.findViewById(com.android.internal.R.id.actions);
    		if(notificationActions != null){
        				if (notificationActions.getParent() != null
        						&& notificationActions.getParent() instanceof FrameLayout) {
        					FrameLayout fr = (FrameLayout) notificationActions.getParent();
        					fr.setBackgroundColor(Color.argb(0xcc, 0xee, 0xee, 0xee));
       		 	for (int i = 0; i < notificationActions.getChildCount(); i++) {
       		 		if (notificationActions.getChildAt(i) instanceof Button) {
       		 			notificationAction = (Button) notificationActions.getChildAt(i);
       		 		    //ANROID_712_UPDATE: modify by ty on 20170221 for android7.1 update
        				int color = res.getColor(android.R.color.secondary_text_light, null);//secondary_text_material_light
    		 			notificationAction.setTextColor(color);
        						}
        					}
        				}
               		 }
				...
				if (contentView.findViewById(com.android.internal.R.id.actions) instanceof LinearLayout) {
					notificationActions = (LinearLayout) contentView
							.findViewById(com.android.internal.R.id.actions);
					
					Notification n = sbn.getNotification();
					List<Notification.Action> actions = null;
					Notification.Action actions1[] = null;
					if(n != null){
						actions1 = n.actions;
				        final int numActions1 = actions1.length;
					}
					
					if (notificationActions != null) {//
						for (int i = 0; i < notificationActions.getChildCount(); i++) {
							if (notificationActions.getChildAt(i) instanceof Button) {
								notificationAction = (Button) notificationActions
										.getChildAt(i);
								String text = notificationAction.getText().toString();
								
								int color = res.getColor(R.color.notify_default_color_action, null);
								//notificationAction.setTextColor(Color.parseColor("#FF0000"));
								if (text.equalsIgnoreCase(res.getString(R.string.volumeui_prompt_deny))) {
									color = res.getColor(R.color.notify_default_color_action_reject, null);
								}
								notificationAction.setTextColor(color);
								
								if(notificationActions.getChildCount() == 2){
									LinearLayout.LayoutParams lp = //(LinearLayout.LayoutParams)notificationAction.getLayoutParams();
									new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, 0f);
									lp.width = Utilities.dipToPixel(mContext, 120);
									
									if(i == 0){
										lp.setMargins(Utilities.dipToPixel(mContext, 40), 0, Utilities.dipToPixel(mContext, 40), 0);
										//notificationAction.setPadding(Utilities.dipToPixel(mContext, 80), 0, 0, 0);
									}else{
										lp.setMargins(0, 0, 
											 	Utilities.dipToPixel(mContext, 40), 0);
										//notificationAction.setPadding(Utilities.dipToPixel(mContext, 50), 0, 0, 0);
									}
									notificationAction.setLayoutParams(lp);
								}else if(notificationActions.getChildCount() == 1){
									LinearLayout.LayoutParams lp = //(LinearLayout.LayoutParams)notificationAction.getLayoutParams();
									new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT, 1.0f);
									lp.setMargins(Utilities.dipToPixel(mContext, 80), 0, Utilities.dipToPixel(mContext, 80), 0);
									notificationAction.setLayoutParams(lp);
								}
  
								if(actions1 != null && actions1[i] != null){
									//notificationAction.setCompoundDrawablesWithIntrinsicBounds(actions1[i].getIcon().getResId(), 0, 0, 0);
									notificationAction.setGravity(Gravity.CENTER);
								}
								
								notificationAction.setBackground(null);
								//notificationAction.setBackgroundDrawable(mContext.getDrawable(R.drawable.round_rectangle));
							}
						}
    				}
					notificationActions.setBackgroundColor(res.getColor(R.color.notify_actions_background_color, null));
//					notificationActions.setBackground(null);
					notificationActions.setGravity(Gravity.TOP|Gravity.CENTER);//_HORIZONTAL
       		 	}
       		 }
        } catch (Exception e) {
		}
    ...
    private final class NotificationClicker implements View.OnClickListener {
        public void onClick(final View v) {
			...
			int notificationUser = intent == null ? 0 : intent.getCreatorUserHandle().getIdentifier();
			Log.d(TAG, "Clicked on notification intended for user: " + notificationUser);

			UserManager userManager = mContext.getSystemService(UserManager.class);
			final UserInfo userInfo = userManager.getUserInfo(notificationUser);

			if (intent != null && notificationUser != mCurrentUserId && notificationUser != UserHandle.USER_ALL
					&& userInfo != null && !userInfo.partial) {
				Log.d(TAG, "Current User does not match the Notification user. Switching Users now...");
				try {
					ActivityManagerNative.getDefault().switchUser(notificationUser);
					if (!mStatusBarKeyguardViewManager.isSecure(notificationUser)) {
                ...
                public boolean onDismiss() {
                    ...
                    new Thread() {
                        ...
                        public void run() {
                            ...
                            LogHelper.sd(TAG, "700007_a onNotificationClick pkg="+sbn.getPackageName());
                            try {
                                mBarService.onNotificationClick(notificationKey);
                            } catch (RemoteException ex) {
                                // system process is dead if we're here.
                            	LogHelper.sd(TAG, "700007_a1 onNotificationClick pkg="+sbn.getPackageName());
                            }
    ...
    protected NotificationData.Entry createNotificationViews(StatusBarNotification sbn) {
        ...
        iconView.setNotificationIcon(true);
...
/*    public static Bitmap getPackageThemeBitmapNotification(String packageName, int userId){
    	Bitmap newBp = null;
        if(!NotificationCollapseManage.getDefault(mContext).isUsedSelfNotificationPng(packageName)){
     		
     		Bitmap bp= StatusBarIconView.getPackageThemeBitmap(mContext, packageName, 
    ...
    public StatusBarIconView createIcon(StatusBarNotification sbn) {
        // Construct the icon.
        Notification n = sbn.getNotification();
        final StatusBarIconView iconView = new StatusBarIconView(mContext, sbn, sbn.getPackageName(),
                sbn.getPackageName() + "/0x" + Integer.toHexString(sbn.getId()), n);
        iconView.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
        iconView.updateStatusIconColor(getStatusBarIconColor());
    ...
    protected View mQuickSetting;
    protected ImageView mQuickSettingsButton;
    public Boolean mShowQuickSettings = false;
    /**
    ...
    protected void updateRowStates() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (suppressedSummary || (isLockscreenPublicMode() && !mShowLockscreenNotifications) ||
            		(mShowQuickSettings && Utilities.showDragDownQuickSettings()) ||//!isLockscreenPublicMode() && 
                    (onKeyguard && !childWithVisibleSummary
                            && (visibleNotifications >= maxNotifications || !showOnKeyguard))) {
                entry.row.setVisibility(View.GONE);
                if (onKeyguard && showOnKeyguard && !childNotification && !suppressedSummary && showNotificationsWhenLocked(entry)) {
    ...
    protected abstract boolean showNotificationsWhenLocked(Entry ent);
    public abstract boolean shouldDisableNavbarGestures();
    public abstract boolean hasLockTheme();
        ...
        if (applyInPlace) {
            ...
            try {
                ...
                if(!isCoolShowThemeMode()){
                	entry.showWhenLocked = true;//modify by ty
                }
    ...
    private void updateNotificationViews(Entry entry, StatusBarNotification sbn) {
        ...
        if (contentView != null && entry.getContentView() != null) {
        	setNotificationStyle(entry.getContentView(),sbn,1);
        }
        if (bigContentView != null && entry.getExpandedContentView() != null) {
        	bigContentView.reapply(sbn.getPackageContext(mContext),
                    entry.getExpandedContentView(),
                    mOnClickHandler);
            setNotificationStyle(entry.getExpandedContentView(),sbn,2);
        }
        ...
        if (headsUpContentView != null && headsUpChild != null) {
            ...
            setNotificationStyle(headsUpChild,sbn,3);
        }
        if (publicContentView != null && entry.getPublicContentView() != null) {
        	publicContentView.reapply(sbn.getPackageContext(mContext),
                    entry.getPublicContentView(), mOnClickHandler);
            setNotificationStyle(entry.getPublicContentView(),sbn,4);
        }
    ...
    protected boolean shouldPeek(Entry entry, StatusBarNotification sbn) {
    	...
    	boolean updateTicker = sbn.getNotification().tickerText != null;
        boolean isForCurrentUser = isNotificationForCurrentProfiles(sbn);
    	isTicker = updateTicker && isForCurrentUser;
    ...
    public boolean isKeyguard(){
    	//Log.d(TAG,"isKeyguard mState="+mState);
    	//===modify by ty
    	return mState == StatusBarState.KEYGUARD;
    	//return false;//(mState == StatusBarState.KEYGUARD || mState == StatusBarState.SHADE_LOCKED);
    }

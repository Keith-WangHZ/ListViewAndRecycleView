...
public class NavigationBarInflaterView extends FrameLayout implements TunerService.Tunable {
    ...
    protected String getDefaultLayout() {
        return mContext.getString(R.string.config_navBarLayout_3);
    }
...
//        }
    ...
    public void resetLayout(String newValue){
    	 if (!Objects.equals(mCurrentLayout, newValue)) {
             clearViews();
             inflateLayout(newValue);
             requestLayout();
             invalidate();
             LogHelper.sd(TAG,"NavigationBar newValue="+newValue+" mCurrentLayout="+mCurrentLayout);
         }
    }
...
//        LogHelper.sd(TAG,"NavigationBar button="+button+" size="+size);
        View v = null;
        if (HOME.equals(button)) {
            ...
            v = inflater.inflate(R.layout.menu, parent, false);
            if (landscape && isSw600Dp()) {
                setupLandButton(v);
            }

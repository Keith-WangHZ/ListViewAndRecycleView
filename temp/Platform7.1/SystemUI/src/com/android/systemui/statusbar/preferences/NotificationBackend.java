...
public class NotificationBackend {
    ...
    public AppRow loadAppRow(Context context, PackageManager pm, ApplicationInfo app) {
        ...
        row.banned = getNotificationsBanned(row.pkg, row.uid);//已屏蔽
        row.appImportance = getImportance(row.pkg, row.uid);//显示时不发出提示音//需要移植
        row.appBypassDnd = getBypassZenMode(row.pkg, row.uid);//覆盖勿扰设置//需要移植
        row.appVisOverride = getVisibilityOverride(row.pkg, row.uid);//屏幕锁定时//需要移植
        row.lockScreenSecure = new LockPatternUtils(context).isSecure(//屏幕锁定时
                UserHandle.myUserId());
        return row;
    }
    ...
    public static class AppRow extends Row {
        ...
        public boolean banned;//已屏蔽
        public boolean first;  // first app in section
        public boolean systemApp;
        public int appImportance;//显示时不发出提示音
        public boolean appBypassDnd;//覆盖勿扰设置
      //屏幕锁定时一律不显示 FILTER_APP_NOTIFICATION_HIDE_ALL == lockScreenSecure && appVisOverride==VISIBILITY_SECRET
        public int appVisOverride;
      //屏幕锁定时不显示敏感内容 FILTER_APP_NOTIFICATION_HIDE_SENSITIVE == lockScreenSecure && appVisOverride==VISIBILITY_PRIVATE

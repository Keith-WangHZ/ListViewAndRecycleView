...
import com.android.keyguard.AntitheftUnlockScreen;
...
public class StatusBarWindowView extends FrameLayout {
    ...
    public StatusBarWindowView(Context context, AttributeSet attrs) {
		...
		mTouchProtectScreen = TouchProtectScreen.getInstance(mContext); 
    }
    ...
    protected void onFinishInflate() {
        ...
        mLockThemeFrame = (FrameLayout) findViewById(R.id.lock_theme_frame);
    }
...
//        if (mTouchProtectScreen.isSensorBeCoveredAndNotShowing()) {
//        	mTouchProtectScreen.onUserActivity();
//        	mService.userActivity();
//        	return true;
//        }
        if (isLockThemeShowing() && mAntitheftUnlockScreen == null  && mService.getBarState() == StatusBarState.KEYGUARD) {
			mService.userActivity();
			return mLockThemeFrame.dispatchTouchEvent(ev);
		}
	...
	private boolean isLockThemeShowing() {
		return mLockThemeFrame.getVisibility() == View.VISIBLE && !mService.isBouncerShowing();
	}
    ...
    //add by mengludong, 2016.08.16
    private TouchProtectScreen mTouchProtectScreen;
    private AntitheftUnlockScreen mAntitheftUnlockScreen;
    private FrameLayout mLockThemeFrame;
    
    KeyguardUpdateMonitorCallback mUpdateCallback = new KeyguardUpdateMonitorCallback() {
		@Override
		public void onAntitheftStateChanged(boolean showing) {
			if (showing) {
				Log.v(TAG, "is Antitheft pattern, ready to show.");
				if (mAntitheftUnlockScreen == null) {
					mAntitheftUnlockScreen = new AntitheftUnlockScreen(mContext, null);
					StatusBarWindowView.this.addView(mAntitheftUnlockScreen);
				}
			} else {
				if (mAntitheftUnlockScreen != null) {
					StatusBarWindowView.this.removeView(mAntitheftUnlockScreen);
					mAntitheftUnlockScreen = null;
	...
	public void onKeyguardOccluded(boolean occluded) {
        ...
        if (occluded) {
            mAntitheftUnlockScreen.setVisibility(GONE);
        } else {
            mAntitheftUnlockScreen.setVisibility(VISIBLE);
        }
    }
	
	public void setLockThemeVisible(boolean showTheme, View themeView) {
		if (showTheme && themeView != null) {
			Log.v(TAG, "showing lock theme");
			if (themeView.getParent() != mLockThemeFrame) {
				mLockThemeFrame.removeAllViews();
				mLockThemeFrame.addView(themeView);
			}
			mLockThemeFrame.setVisibility(View.VISIBLE);
		} else {
		    Log.v(TAG, "hiding lock theme");
			mLockThemeFrame.setVisibility(View.GONE);
			mLockThemeFrame.removeAllViews();
		}
	}
	//add end

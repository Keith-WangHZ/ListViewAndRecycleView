...
public class StatusBarKeyguardViewManager implements RemoteInputController.Callback {
    ...
    public void registerStatusBar(PhoneStatusBar phoneStatusBar,
        ...
        mThemeManager = new StatusBarThemeViewManager(mContext, this);
    }
    ...
    protected void showBouncerOrKeyguard() {
        if (mBouncer.needsFullscreenBouncer()) {
            ...
            mBouncer.prepare();
            if (mHasLockTheme) {
			    mPhoneStatusBar.updateLockThemeVisible(true);
			    mThemeManager.show();
			}
        }
    }

    private void showBouncer(int position) {
        if (mShowing) {
            mBouncer.show(false /* resetSecuritySelection */, position);
        }
        updateStates();
    }
     ...
     */
    public void reset() {
    	Log.d(TAG, "mShowing = " + mShowing + ", mOccluded = " + mOccluded);
        if (mShowing) {
            if (mOccluded) {
                mPhoneStatusBar.hideKeyguard(false);
    ...
    public void setOccluded(boolean occluded) {
        ...
        mOccluded = occluded;
        mPhoneStatusBar.onKeyguardOccluded(occluded);
    ...
    public void hide(long startTime, final long fadeoutDuration) {
        ...
        if (mPhoneStatusBar.isInLaunchTransition() ) {
            ...
            if (mFingerprintUnlockController.getMode()
            	...
            	Log.d("fingerWakeupSlowLog:", "hideKeyguard() begin");
                mFingerprintUnlockController.startKeyguardFadingAway();
                mPhoneStatusBar.setKeyguardFadingAway(startTime, delay, fadeoutDuration);
                Log.d("fingerWakeupSlowLog:", "hideKeyguard() before");
                boolean staying = mPhoneStatusBar.hideKeyguard();
                Log.d("fingerWakeupSlowLog:", "hideKeyguard() end");
                if (!staying) {
                    mStatusBarWindowManager.setKeyguardFadingAway(true);
                    if (mFingerprintUnlockController.getMode()
            ...
            Log.d("fingerWakeupSlowLog:", "hideKeyguard() one");
            mStatusBarWindowManager.setKeyguardShowing(false);
            Log.d("fingerWakeupSlowLog:", "hideKeyguard() two");
            mBouncer.hide(true /* destroyView */);
            Log.d("fingerWakeupSlowLog:", "hideKeyguard() three");
            mViewMediatorCallback.keyguardGone();
            executeAfterKeyguardGoneAction();
            updateStates();
            Log.d("fingerWakeupSlowLog:", "hideKeyguard() four");
        }
    }
     ...
     */
    public void dismiss() {
        dismiss(KeyguardHostView.POSITION_NORMAL);
    }
    ...
    public void dismiss(int position) {
        if (mDeviceInteractive || mDeviceWillWakeUp) {
            ...
            if (mHasLockTheme) {
				if (mThemeManager.isVlifeTheme()) {
					mPhoneStatusBar.updateLockThemeVisible(false);
				}
				mThemeManager.hide();
			}
        }
	...
	public boolean isSimPinOrSimPuk() {
		return mBouncer.needsFullscreenBouncer();
	}
	
	private boolean mCovered;
	private StatusBarThemeViewManager mThemeManager;
    private boolean mHasLockTheme;
	...
	public void setCovered(boolean covered) {
		mCovered = covered;
		mPhoneStatusBar.updateCoverState(covered);
		setLockTheme(null, false, false);
	}
	// add end

	/**
	 * Set lock theme if theme files can be load
	 * @param path
	 * @param reload 
	 * @param isVlife 
	 * @add by mengludong 2015.12.03
	 */
    public void setLockTheme(String path, boolean reload, boolean isVlife) {
        Log.v(TAG, "setLockTheme() path = " + path + ", mIsCoolShowTheme = " + mHasLockTheme + ", reload = " + reload
                + ", isVlife = " + isVlife);
        mThemeManager.loadThemeView(path, reload, isVlife);
    }
    
    public void onLockThemeLoaded(boolean loaded) {
        Log.v(TAG, "setLockTheme() loaded ? " + loaded);
        if (loaded != mHasLockTheme) {
        	KeyguardUpdateMonitor.getInstance(mContext).cancelToast();
            mHasLockTheme = loaded;
            mPhoneStatusBar.onLockThemeChanged(mHasLockTheme);
			if (mShowing) {
				mPhoneStatusBar.updateLockThemeVisible(mHasLockTheme);
				if (mHasLockTheme) {
					mThemeManager.show();
				} else {
					mThemeManager.hide();
        ...
        mViewMediatorCallback.resetKeyguard(false/* relock */);
    }
    
    public boolean isCoolShowTheme() {
		return mHasLockTheme;
	}
	...
	public View getLockThemeView() {
		return mThemeManager.getThemeView();
	}
	...
	public Drawable getThemeBackground() {
		...
		return mThemeManager.getThemeBackground();
	}

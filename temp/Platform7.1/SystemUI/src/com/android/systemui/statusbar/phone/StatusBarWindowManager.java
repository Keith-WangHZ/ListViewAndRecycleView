...
public class StatusBarWindowManager implements RemoteInputController.Callback {
	...
	public StatusBarWindowManager(Context context) {
		...
		mScreenBrightnessDoze = mContext.getResources().getInteger(com.android.internal.R.integer.config_screenBrightnessDoze) / 255f;
	}

	private boolean shouldEnableKeyguardScreenRotation() {
		Resources res = mContext.getResources();
		Log.d("eee","SystemProperties.getBoolean(lockscreen.rot_override, false)="+SystemProperties.getBoolean("lockscreen.rot_override", false)
	 ...
	 *            The view to add.
	 * @param barHeight
	 *            The height of the status bar in collapsed state.
	 */
	public void add(View statusBarView, int barHeight) {
		...
		mLp = new WindowManager.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, barHeight,
				WindowManager.LayoutParams.TYPE_STATUS_BAR, WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE
						| WindowManager.LayoutParams.FLAG_TOUCHABLE_WHEN_WAKING | WindowManager.LayoutParams.FLAG_SPLIT_TOUCH
	...
	private void adjustScreenOrientation(State state) {
		if (state.isKeyguardShowingAndNotOccluded()) {
			...
			Log.d("eee","...mLpChanged.screenOrientation="+mLpChanged.screenOrientation);
		} else {
	...
	private void applyFocusableFlag(State state) {
		...
		if (state.keyguardShowing && state.keyguardNeedsInput && state.bouncerShowing || BaseStatusBar.ENABLE_REMOTE_INPUT
	...
	private boolean isExpanded(State state) {
		return !state.forceCollapsed
				&& (state.isKeyguardShowingAndNotOccluded() || state.panelVisible || state.keyguardFadingAway
						|| state.bouncerShowing || state.headsUpShowing);
	}
	...
	private void applyUserActivityTimeout(State state) {
		if (state.isKeyguardShowingAndNotOccluded() && state.statusBarState == StatusBarState.KEYGUARD && !state.qsExpanded) {
	...
	private void applyForceStatusBarVisibleFlag(State state) {
		if (state.forceStatusBarVisible) {
			mLpChanged.privateFlags |= WindowManager.LayoutParams.PRIVATE_FLAG_FORCE_STATUS_BAR_VISIBLE_TRANSPARENT;
		} else {
			mLpChanged.privateFlags &= ~WindowManager.LayoutParams.PRIVATE_FLAG_FORCE_STATUS_BAR_VISIBLE_TRANSPARENT;
		}
	}
	...
	/**
	 * Force the window to be collapsed, even if it should theoretically be
	 * expanded. Used for when a heads-up comes in but we still need to wait for
	 * the touchable regions to be computed.
	 */
	...
	/**
	 * Set whether the screen brightness is forced to the value we use for doze
	 * mode by the status bar window.
	 */

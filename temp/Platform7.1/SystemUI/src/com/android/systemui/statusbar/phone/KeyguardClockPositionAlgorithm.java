...
public class KeyguardClockPositionAlgorithm {
	...
	private int mStackScrollerPadding = 75;
    ...
    public void loadDimens(Resources res) {
        ...
        mStackScrollerPadding = (int) (res.getDimensionPixelSize(R.dimen.status_bar_height) + 55 * mDensity);
    }
    ...
    public void setup(int maxKeyguardNotifications, int maxPanelHeight, float expandedHeight,
        ...
        if (DEBUG) {
			Log.v(TAG, "mMaxKeyguardNotifications = " + mMaxKeyguardNotifications + ", mMaxPanelHeight = " + mMaxPanelHeight
					+ ", mExpandedHeight" + mExpandedHeight + ", mNotificationCount" + mNotificationCount + ", mHeight" + mHeight
					+ ", mKeyguardStatusHeight = " + mKeyguardStatusHeight + ", mEmptyDragAmount = " + mEmptyDragAmount);
		}
    }
    ...
    public void run(Result result) {
		int y = (int) (mHeight - mKeyguardStatusHeight - (mMaxPanelHeight - mExpandedHeight) / 2 + mEmptyDragAmount);
        result.stackScrollerPaddingAdjustment = 0;
        result.clockY = y;
        result.stackScrollerPadding = mStackScrollerPadding;
        result.clockScale = 1.0f;
        result.clockAlpha = 1.0f;
        if (DEBUG) {
			Log.v(TAG, "result.clockY = " + result.clockY + ", result.stackScrollerPadding = " + result.stackScrollerPadding
					+ ", result.clockScale" + result.clockScale + ", result.clockAlpha" + result.clockAlpha);
		}
    }

...
public abstract class Ticker {
    ...
    private final class Segment {
        ...
        StaticLayout getLayout(CharSequence substr) {
            ...
            return new StaticLayout(substr, mPaint, w < 0 ? 0 : w, Alignment.ALIGN_NORMAL, 1, 0, true);//yaolihui@yulong.com_20160818:crash here with Bug 491796
        }
    ...
    public void addEntry(StatusBarNotification n) {
        ...
        if (initialCount > 0) {
            ...
            if (n.getPackageName().equals(seg.notification.getPackageName())
                    && n.getNotification().getSmallIcon() == seg.notification.getNotification().getSmallIcon()
        ...
        final Drawable icon = StatusBarIconView.getIcon(mContext,
                new StatusBarIcon(n.getUser(), n.getPackageName(), n.getNotification().getSmallIcon(), n.getNotification().iconLevel, 0,
                        n.getNotification().tickerText));
    ...
    public boolean isNotificationReversedColorNeeded(){
		String pkg = mPkg;
//		Log.d("","NotificationIcon headsup pkg="+pkg);
		boolean b = ((NotificationCollapseManage.getDefault(mContext)
	...
	public void updateTickerColor(int iconColor) {
		if (mIconColor != iconColor) {
			mIconColor = iconColor;
			onUpdateTickerColor();
		}
	}
	public void onUpdateTickerColor(){
		for (int i = 0; i < mTextSwitcher.getChildCount(); i++) {
			TextView text = (TextView) mTextSwitcher.getChildAt(i);
			if (text != null) {
				text.setTextColor(mIconColor);
			}
		}
		Log.d("","NotificationIcon headsup11 pkg="+mPkg);
		for (int i = 0; i < mIconSwitcher.getChildCount(); i++) {
			ImageView v = (ImageView) mIconSwitcher.getChildAt(i);
			if (v != null && isNotificationReversedColorNeeded()) {
				v.clearColorFilter();
				v.setColorFilter(mIconColor, PorterDuff.Mode.SRC_IN);
			}else if (v != null){
				v.clearColorFilter();
			}
		}

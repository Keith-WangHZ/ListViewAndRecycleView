...
public class NotificationPanelView extends PanelView implements
    ...
    public KeyguardAffordanceHelper mAfforanceHelper;
    ...
    private TextView mOwnerInfoIndication;
    ...
    View mPanelCarrierText;
    View mNotificationLineCarrier;
	View mNotificationLineTopStackScroller;
...
//        updateGaussWindowBlurParams();
//		setBlurMode(BlurParams.BLUR_MODE_WINDOW);
//		setBackground(null);
    }
    
    protected void updateGaussWindowBlurParams() {
        float expandFraction = getExpandedFraction();
//        setBlurRadiusDp(12f * expandFraction);
////        LogHelper.sd(TAG,"2000011 mKeyguardShowing="+mKeyguardShowing+" expandFraction="+expandFraction);
//        setBlurAlpha(mKeyguardShowing ? 0 : 0.8f * expandFraction);
        
        IvviGaussBlurViewFeature.setBlurRadiusDp(this, 12f * expandFraction);
		IvviGaussBlurViewFeature.setBlurAlpha(this, mKeyguardShowing ? 0 : 0.8f * expandFraction);
    }

    public void setStatusBar(PhoneStatusBar bar) {
        mStatusBar = bar;
    }
    private static final String KEYGUARD_WALLPAPER_CHANGED = "android.intent.action.KEYGUARD_WALLPAPER_CHANGED";

    private boolean mIsPrimary = true;
	protected boolean mIsSpuerSavePowerMode;

    private void register() {
        final IntentFilter filter = new IntentFilter();
        filter.addAction(KEYGUARD_WALLPAPER_CHANGED);
        filter.addAction(ACTION_SUPER_SAVING_POWER);
        mContext.registerReceiver(new BroadcastReceiver() {
			@Override
            public void onReceive(Context context, Intent intent) {
                final String action = intent.getAction();
                Log.d(TAG, "KeyguardUViewManager action = " + action);
                if (KEYGUARD_WALLPAPER_CHANGED.equals(action)) {
                } else if (action.equals(ACTION_SUPER_SAVING_POWER)) {
                    mIsSpuerSavePowerMode = intent.getBooleanExtra("state", false);
                    Log.d(TAG, "ACTION_SUPER_SAVING_POWER state = " + mIsSpuerSavePowerMode);
                    if (mStatusBar != null && mIsSpuerSavePowerMode&& mStatusBar.getBarState() == StatusBarState.SHADE) {
                        mStatusBar.makeExpandedInvisible();
                    }
                }
            }
        }, filter);
    ...
    protected void onFinishInflate() {
        ...
        mOwnerInfoIndication = (TextView)findViewById(R.id.owner_info_indication);
        mPanelCarrierText = findViewById(R.id.carrier_label);
        mNotificationLineCarrier = findViewById(R.id.notification_line_carrier);
		mNotificationLineTopStackScroller = findViewById(R.id.notification_line_top_stack_scroller);
		mQuickSetting = findViewById(R.id.setting_config_container0);
    }
    ...
    public void setOwnerInfoIndication(String ownerinfo){
    	if(ownerinfo != null && !ownerinfo.equals("")){
    		mOwnerInfoIndication.setVisibility(View.VISIBLE);
    		mOwnerInfoIndication.setText(ownerinfo);
    	}else{
    		mOwnerInfoIndication.setVisibility(View.GONE);
    	}
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
		if (mBlockTouches || mQsContainer.isCustomizing()
				|| (mIsSpuerSavePowerMode && mStatusBar.getBarState() == StatusBarState.SHADE)
				|| (KeyguardUpdateMonitor.getInstance(mContext).isOccluded() && KeyguardUpdateMonitor.getInstance(mContext).isKeyguardShowing())) {
			if (mIsSpuerSavePowerMode && mStatusBar.getBarState() == StatusBarState.SHADE) {
				LogHelper.sd(TAG, "onTouchEvent mIsSpuerSavePowerMode = " + mIsSpuerSavePowerMode);
			}
				LogHelper.sd(TAG,
						"onTouchEvent Keyguard isOccluded = " + KeyguardUpdateMonitor.getInstance(mContext).isOccluded()
								+ ", mKeyguardShowing = "
								+ KeyguardUpdateMonitor.getInstance(mContext).isKeyguardShowing());
			return false;
		}
        initDownStates(event);
    ...
    public void setBarState(int statusBarState, boolean keyguardFadingAway,
        ...
        //mStatusBar.mNotificationBlackFrame.setVisibility(mKeyguardShowing?View.GONE:View.VISIBLE);
        ...
        if (goingToFullShade || (oldState == StatusBarState.KEYGUARD
            ...
            mNotificationLineCarrier.setVisibility(View.GONE);
            mNotificationLineTopStackScroller.setVisibility(View.GONE);
            ...
            if(mKeyguardShowing){
            	...
            	mNotificationLineCarrier.setVisibility(View.VISIBLE);
            }
            if(keyguardShowing){
            	mNotificationLineTopStackScroller.setVisibility(View.GONE);
            }else{
            	mNotificationLineTopStackScroller.setVisibility(View.VISIBLE);
            }
    ...
    private void setKeyguardBottomAreaVisibility(int statusBarState,
        ...
        if (statusBarState == StatusBarState.KEYGUARD && mStatusBar.hasLockTheme()) {
        	mKeyguardBottomArea.setVisibility(View.GONE);
    ...
    private void setQsExpansion(float height) {
        ...
        height = mQsMinExpansionHeight;//===modify by ty
    ...
    protected void updateQsExpansion() {//
    	if(Utilities.showDragDownQuickSettings()){
        	final float translationScaleY = getQsExpansionFraction() - 1;//===modify by ty
//        	Log.d("","500005 transY="+translationScaleY+" y2="+getHeaderTranslation());
//        	mQuickSetting.setTranslationY(Utilities.dipToPixel(mContext, 72)+
//        			translationScaleY * mQuickSetting.getHeight());
        	mQuickSetting.setTranslationY(Utilities.dipToPixel(mContext, 12)+
        			getHeaderTranslation()*2);
    	}
    ...
    private void updateHeaderShade() {
    	//Log.v("trace", "updateHeaderShade()", new Throwable().fillInStackTrace());
        if (true) {//!mHeaderAnimatingIn
            // mQsContainer.setY(height - mQsContainer.getHeight() + getHeaderTranslation());
            float headY;
            float headerHeight = mQsContainer.getHeight();//getHeaderTranslation();//mStatusBarExpanderHeaderQsPanelMerger.getHeight();
            float topNotificationY = mNotificationStackScroller.getNotificationsTopY();
            //LogHelper.sd(TAG,"headerHeight="+headerHeight+"trans="+getHeaderTranslation());
            if (mNotificationStackScroller.isNotificationExistVisibleChild()) {
                headY = Math
                        .min((topNotificationY - headerHeight) * 0.5f, 0);
            } else {
                headY = -(1 - getExpandedFraction()) * headerHeight;
            }
            mNotificationLineTopStackScroller.setY(headY+headerHeight);
            
            if (headerHeight >= (topNotificationY + Utilities.dipToPixel(mContext, 15))) {
            	mNotificationLineTopStackScroller.setVisibility(GONE);
			} else {
				mNotificationLineTopStackScroller.setVisibility(VISIBLE);
			}
            
            float expandFraction = getExpandedFraction();
            mNotificationLineCarrier.setAlpha(expandFraction);
			
            mPanelCarrierText.setAlpha(expandFraction);
            /*float expandFraction = getExpandedFraction();
            mStatusbarKeyguardCarrierText.setY(mStatusbarKeyguardCarrierTextY * expandFraction);
            mNotificationLineCarrier.setY(mStatusbarKeyguardCarrierTextY * expandFraction - Utilities.dipToPixel(mContext, 5));
            ExpandableView firstChild = mNotificationStackScroller.getFirstExpandableViewNotGone();
            if (firstChild != null) {
                firstChild.setClipTopAmount(Math.max(0,
                        (int) (mStatusBarExpanderHeaderQsPanelMerger.getBottom() - mNotificationStackScroller
    ...
    private void updateHeaderKeyguardAlpha() {
        ...
        mKeyguardStatusBar.setAlpha(Math.max(1 - getQsExpansionFraction() * 2, 0f)/*Math.min(getKeyguardContentsAlpha(), alphaQsExpansion)
                * mKeyguardStatusBarAnimateAlpha*/);
        mKeyguardStatusView.setAlpha(Math.min(getKeyguardContentsAlpha(), alphaQsExpansion)
                * mKeyguardStatusBarAnimateAlpha);
...
//        }
    ...
    public void updateEmptyShadeView() {
    ...
    public void updateYLKeyguardViewVisibility(int oldBarState, int newBarState, boolean themeStateChanged, boolean bouncerJustShowed) {
		Log.v(TAG, "updateYLKeyguardViewVisibility() oldBarState = " + oldBarState + ", newBarState = " + newBarState
				+ ", themeStateChanged = " + themeStateChanged + ", bouncerJustShowed = " + bouncerJustShowed);
        boolean keyguardShowingOld = oldBarState == StatusBarState.KEYGUARD || oldBarState == StatusBarState.SHADE_LOCKED;
        boolean keyguardShowingNew = newBarState == StatusBarState.KEYGUARD || newBarState == StatusBarState.SHADE_LOCKED;
		boolean keyguardShowingStateChanged = keyguardShowingNew != keyguardShowingOld || bouncerJustShowed;
        boolean isCoolShowThemeMode = mStatusBar.isCoolShowThemeMode();
//        mBackground.setAnimationEnabled(keyguardShowingNew && !isCoolShowThemeMode && mKeyguardStyleHostView.getKeyguardStyleType() == KeyguardStyleType.SIMPLE);

        if (themeStateChanged) {
            onLockThemeChanged(keyguardShowingNew);
        }

        if (keyguardShowingStateChanged) {
			onKeyguardShowingStateChanged(keyguardShowingNew && !isCoolShowThemeMode, bouncerJustShowed);
        }
    }
    
    private void onKeyguardShowingStateChanged(boolean keyguardShowing, boolean bouncerJustShowed) {
//        if (keyguardShowing) {
//        	mBackground.setVisibility(View.VISIBLE);
//        	mKeyguardStyleHostView.setViewVisibility();
//        } else {
//            mBackground.setVisibility(View.GONE);
//            mKeyguardStyleHostView.setVisibility(View.GONE);
//        }
        if (bouncerJustShowed) {
            mKeyguardStatusView.setVisibility(View.GONE);
            mKeyguardBottomArea.setVisibility(View.GONE);
		}
    }

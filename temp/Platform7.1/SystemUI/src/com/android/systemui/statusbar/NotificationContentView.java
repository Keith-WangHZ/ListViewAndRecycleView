...
public class NotificationContentView extends FrameLayout {
    ...
    public int mOwnHeight = 0;
    @Override
    public void onDraw(Canvas c) {
        super.onDraw(c);
        
        /*int width = mWindowManager.getDefaultDisplay().getWidth();//===modify by ty for Notification
        int height = mWindowManager.getDefaultDisplay().getHeight();
        int startX = Utilities.dipToPixel(getContext(), 16);
        int endX = width-startX;
//        String pkg = mStatusBarNotification.getPackageName();
//        LogHelper.sd("","onDraw w="+width+" h="+height+" sx="+startX+" ex="+endX+
//        		" mOwnHeight="+mOwnHeight+" pkg="+pkg);
        mOwnHeight = mContentHeight;
        if(mOwnHeight > 0){
        	mOwnHeight = mOwnHeight-1;
        }
        final Paint pt = new Paint(Paint.ANTI_ALIAS_FLAG);
        pt.setColor(getResources().getColor(R.color.notify_line_default_color));
        pt.setStyle(Paint.Style.STROKE);
        pt.setStrokeWidth(1);
        c.drawLine(startX, mOwnHeight, endX, mOwnHeight, pt);*/
    }
...
//        mContainingNotification.setContentBackground(customBackgroundColor, animate, this);
    ...
    private void updateViewVisibilities(int visibleType) {
        ...
        if (mExpandedChild != null) {
            ...
            //mSetHeight = expandedVisible? mOwnHeight:mContentHeight;
        }
        if (mHeadsUpChild != null) {

...
public class TaskView extends FixedSizeFrameLayout implements Task.TaskCallbacks,
    ...
    public void setAnimationProgress(float progress){
    	this.setTranslationX(mContext.getResources().getDimensionPixelSize(R.dimen.recents_task_view_remove_anim_translation_x) * 3 * progress);
    ...
    void dismissTask() {
    	...
    	if(RecentsTaskLoader.getInstance().isBaseIntenInLock(getTask().key.baseIntent)){
    		return;
    	}
    ...
    void startDeleteTaskAnimation(final Runnable r) {
        ...
        mContext.getResources().getDimensionPixelSize(R.dimen.recents_task_view_remove_anim_translation_x);
        animate().translationX( mContext.getResources().getDimensionPixelSize(R.dimen.recents_task_view_remove_anim_translation_x))
            .alpha(0f)
            .setStartDelay(0)
    ...
    public void onTaskBound(Task t, boolean touchExplorationEnabled, int displayOrientation,
        ...
        if (!t.isDockable && ssp.hasDockedTask()) {
        	//wanghuazhi modify for split toast show error
            if (mIncompatibleAppToastView == null) {

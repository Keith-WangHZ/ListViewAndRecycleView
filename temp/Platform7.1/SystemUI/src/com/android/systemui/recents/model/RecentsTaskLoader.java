...
import com.android.systemui.recents.LockedListManager;
                        ...
                        if (cachedIcon == null) {
                        	...
                        	if(activityInfo != null){
                     		...
                     		if(activityInfo != null && activityInfo.applicationInfo != null){
                     			activityName = activityInfo.applicationInfo.className;
                     		}
                        	if(packageName != null){
                     			bp = StatusBarIconView.getPackageThemeBitmap(mContext, packageName, activityName, userId);
                            	//Log.w(TAG, "loadThumbnailAndIcon backgroundTask: packageName=" + packageName + "; activityName=" + activityName + "; userId"); //
                     		
                            	if(bp != null){
                            		cachedIcon = new BitmapDrawable(bp);
                            		mIconCache.put(taskKey, cachedIcon);
                            	}
                     		}

                        	if(cachedIcon == null){//load others' icon
                ...
                if (!mCancelled && mLoadQueue.isEmpty()) {
                    synchronized(mLoadQueue) {
                        try {
                            ...
                            mLoadQueue.wait();
//                            Utils.getInstance(SystemUIApplication.getContext()).getAllPss();
//                        	Utils.getInstance(SystemUIApplication.getContext()).getProcessNetwork(); 
...
public class RecentsTaskLoader {
    ...
    BitmapDrawable mDefaultIcon;
    Bitmap mDefaultThumbnail;
    private LockedListManager mLcokedList;
    ...
    public boolean isBaseIntenInLock(Intent baseIntent){
    	if (mLcokedList == null) return false;
    	final ResolveInfo resolveInfo = mSystemServicesProxy.mPm.resolveActivity(baseIntent, 0);
    	boolean ret = false;
    	if(resolveInfo != null && resolveInfo.activityInfo != null){
    	    ret = mLcokedList.isInLockedList(resolveInfo.activityInfo.packageName);
    	}
    	return ret;
    }
    public void setBaseIntentLock(Intent baseIntent,boolean lock){
    	if (mLcokedList == null) return;
    	final ResolveInfo resolveInfo = mSystemServicesProxy.mPm.resolveActivity(baseIntent, 0);
    	if (lock){
    		if(resolveInfo != null && resolveInfo.activityInfo != null){
    		mLcokedList.AddLockedPkg(resolveInfo.activityInfo.packageName);
    		}
    	}else{
    		if(resolveInfo != null && resolveInfo.activityInfo != null){
    		mLcokedList.RemoveLockedPkg(resolveInfo.activityInfo.packageName);
    		}
    	}
    }

    public RecentsTaskLoader(Context context) {
    	context = SystemUIApplication.getContext();
        ...
        if (mLcokedList == null){
        	mLcokedList = new LockedListManager(context, "recents_lock");
        }
    }
    ...
    Drawable getAndUpdateActivityIcon(Context context, Task.TaskKey taskKey, ActivityManager.TaskDescription td,
        ...
        if (loadIfNotCached) {
        	...
        	if(activityInfo != null){
     		...
     		if(packageName != null){
     			bp = StatusBarIconView.getPackageThemeBitmap(context, packageName, activityName, userId);
            	Log.w(TAG, "loadThumbnailAndIcon: packageName=" + packageName + "; activityName=" + activityName + "; userId"); 
     		
            	if(bp != null){
            		icon = new BitmapDrawable(bp);
	...
	public void stopAllTask(Activity activity, Boolean bFinish) {
//		getProcessPid(activity);
//		RecentsConfiguration config = RecentsConfiguration.getInstance();
		List<ActivityManager.RecentTaskInfo> tasks = mSystemServicesProxy.getOriRecentTasks(ActivityManager.getMaxRecentTasksStatic(),
				UserHandle.CURRENT.getIdentifier());
		if (tasks == null) {
			return;
		}
		for (ActivityManager.RecentTaskInfo t : tasks) {
			if (!isBaseIntenInLock(t.baseIntent)) {
				// mSystemServicesProxy.removeTask(t.id, false);
				mSystemServicesProxy.removeTask(t.id);
			}
		}
		if(bFinish){
			activity.finish();

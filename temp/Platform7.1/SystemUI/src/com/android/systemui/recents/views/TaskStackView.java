    ...
    public final void onBusEvent(final DismissAllTaskViewsEvent event) {
        ...
        event.addPostAnimationCallback(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = tasks.size() - 1; i >= 0; i--) {
                	final Task t = tasks.get(i);
                	if(t!=null && !RecentsTaskLoader.getInstance().isBaseIntenInLock(t.key.baseIntent)){
                		
//                		Log.d(TAG, "50000530 kill i="+i+" pkg="+t.key.baseIntent.getComponent().getClassName());
                        
                		EventBus.getDefault().send(new DeleteTaskDataEvent(tasks.get(i)));
                	}else{
//                		Log.d(TAG, "50000530 reserve i="+i+" pkg="+t.key.baseIntent.getComponent().getClassName());
                	}
                }

                MetricsLogger.action(getContext(), MetricsEvent.OVERVIEW_DISMISS_ALL);
                Utils.getInstance(getContext()).setNowUsedMemory();
                
                String noticeClearSuccess = getContext().getString(R.string.clear_memory_notice);
                Toast.makeText(getContext(), noticeClearSuccess, Toast.LENGTH_SHORT).show();
                
                EventBus.getDefault().send(new FinishRecentsViewEvent());
            }
        });
...
//    	}
    ...
    private List<TaskView> mDissmessViews = new LinkedList<TaskView>();
    private ValueAnimator animateDismissAllTask(float totalProgress){
    	ValueAnimator animator = ValueAnimator.ofFloat(0f,totalProgress);
    	animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener(){
    		public void onAnimationUpdate(ValueAnimator animation){
    			Float progress = (Float)animation.getAnimatedValue();
    			LogHelper.se("setAnimationProgress", "setAnimationProgress  progress: " + progress);    	
    			float curViewProgress = progress;
    			float curviewProgressDelta = ANIMATION_DELAY;
    			for(int i=0; i < mDissmessViews.size(); i++){
    				TaskView taskView = mDissmessViews.get(mDissmessViews.size() - i - 1);
    				if (curViewProgress >= 0 && curViewProgress <= 1){
    					taskView.setAnimationProgress(curViewProgress);
    				}
    				curViewProgress -= curviewProgressDelta;
    				curviewProgressDelta -= curviewProgressDelta/3;
    			}
    		}
    	});
    	animator.addListener(new Animator.AnimatorListener() {			
			...
			public void onAnimationEnd(Animator animation) {
				countClearRef++;
				clearAllTask();
				mDissmessViews.clear();
			}			
			@Override
			public void onAnimationCancel(Animator animation) {
				for(TaskView taskView:mDissmessViews){
    ...
    public void stopAllTask(){
    	if (mDissmessViews.size() > 0)
    		return;
    	ArrayList<Task> tasks = mStack.getStackTasks();
    	final long SINGLE_DURATION = 250;
		long delayMillisCount = SINGLE_DURATION;
		long delayMillis = (long)(delayMillisCount * ANIMATION_DELAY);		
        for (int i = 0; i < tasks.size(); i++) {
        	final Task t = tasks.get(i);
			final TaskView taskView = getChildViewForTask(t);
			if(taskView != null && 
					!RecentsTaskLoader.getInstance().isBaseIntenInLock(t.key.baseIntent)){
				//taskView.dismissTask();
				mDissmessViews.add(taskView);
				taskView.setClipViewInStack(false);
	    		delayMillisCount += delayMillis;
	    		delayMillis -= delayMillis*2/3;	    		
			}
        }      
        
        ValueAnimator animator = animateDismissAllTask(delayMillisCount / SINGLE_DURATION);
        animator.setStartDelay(0);
        animator.setDuration(delayMillisCount);

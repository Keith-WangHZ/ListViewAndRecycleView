...
import com.android.systemui.recents.ProgressImageView;
...
public class RecentsView extends FrameLayout {
    ...
    private String  noticeClearSuccess;
    ...
    public RecentsView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        noticeClearSuccess = getContext().getString(R.string.clear_memory_notice);
        ...
        if (RecentsDebugFlags.Static.EnableStackActionButton) {
            ...
            mStackActionButton = (TextView) inflater.inflate(R.layout.recents_stack_action_button,
                    this, false);
            mRecentTextMemory = (TextView)inflater.inflate(R.layout.recents_stack_memory_show, this, false);
...
//            measureChild(mRecentTextMemory,
//                    MeasureSpec.makeMeasureSpec(buttonBounds.width(), MeasureSpec.AT_MOST),
//                    MeasureSpec.makeMeasureSpec(buttonBounds.height(), MeasureSpec.AT_MOST));
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (RecentsDebugFlags.Static.EnableStackActionButton) {
            ...
            mRecentTextMemory.layout(dip2px(mContext, 0), dip2px(mContext, 23), dip2px(mContext, 360),
            		dip2px(mContext, 40));
        }
    ...
    public void stopAllTask(Runnable runnable){
    	//if (mStopAllTask == null){
    		long delayMillisCount = 0;
    		long delayMillisCountMax = 1000;
            int childCount = getChildCount();
            //modify by huazhi for clear all error
            /*if(!isHasLockRecents()){
            	EventBus.getDefault().send(new DismissAllTaskViewsEvent());
            }*/
            int recChild = -1;
            ...
            for (int i = childCount - 1; i >= 0; i--) {
                View child = getChildAt(i);
                if (child instanceof TaskStackView) {
                	final TaskStackView tsv = (TaskStackView)child;
                	int countTask = tsv.getChildCount();
            		long delayMillis = delayMillisCountMax/10;
                	for(int iTaskView = 0; iTaskView < countTask; iTaskView++){
                		View childTask = tsv.getChildAt(iTaskView);
                		if( childTask instanceof TaskView){
                			final TaskView taskView = (TaskView)childTask;
                			if(!RecentsTaskLoader.getInstance().isBaseIntenInLock(taskView.getTask().key.baseIntent)){
	                			RecentsView.this.postDelayed(new Runnable(){ 
	                				public void run(){
//	                					taskView.dismissTask(null);
	                					taskView.dismissTask();
	                				}               				
	                			},delayMillisCount);
	                			if(delayMillisCount < delayMillisCountMax){
        			...
        			},delayMillisCount + 250 + 10);  
        			if(recChild != -1 && recChild==i){
        				lChild = true;
        			}
        			final Boolean bLast = lChild;
        	    	RecentsView.this.postDelayed(new Runnable(){ 
        				public void run(){
        					Toast.makeText(getContext(), noticeClearSuccess, Toast.LENGTH_SHORT).show();
        					RecentsTaskLoader.getInstance().stopAllTask((Activity) getContext(), bLast);
        				}               				
        			},delayMillisCount + 250 + 10);     
                }                
            //}            
    		//mStopAllTask = new StopAllTask();
    		//mStopAllTask.execute(v);          
    	}
            if(!isHasLockRecents()){
            	runnable .run();
            }
//           mActivity.finish();
    ...
    public boolean isHasLockRecents(){
    	boolean isLockRecent = false;
        int childCount = getChildCount();
        for (int i = childCount - 1; i >= 0; i--) {
            View child = getChildAt(i);
            if (child instanceof TaskStackView) {
            	...
            	for(int iTaskView = 0; iTaskView < countTask; iTaskView++){
            		...
            		if( childTask instanceof TaskView){
            			final TaskView taskView = (TaskView)childTask;
            			if(!RecentsTaskLoader.getInstance().isBaseIntenInLock(taskView.getTask().key.baseIntent)){
            				LogHelper.sd("eeee", "isHasLockRecents ........no lock i is done");
        ...
        LogHelper.sd("eeee", "isHasLockRecents ........ isLockRecent = " +isLockRecent);
    	return isLockRecent;
    }
}

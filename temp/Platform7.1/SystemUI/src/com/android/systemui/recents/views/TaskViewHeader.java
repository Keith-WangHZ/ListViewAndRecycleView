...
public class TaskViewHeader extends FrameLayout
    ...
    protected void onFinishInflate() {
        ...
        mLockButton = (ImageView) findViewById(R.id.lock_task);
     ...
     */
    private void updateLayoutParams(View icon, View title, View secondaryButton, View buttonDimiss,View buttonLock) {
...
//        if(buttonLock != null){
//            lp = new FrameLayout.LayoutParams(mHeaderBarHeight, mHeaderBarHeight);
//            buttonDimiss.setLayoutParams(lp);
//            buttonDimiss.setPadding(mHeaderButtonPadding, mHeaderButtonPadding, mHeaderButtonPadding,
...
//                mHeaderButtonPadding);
    ...
    public void onConfigurationChanged() {
        ...
        if (headerBarHeight != mHeaderBarHeight || headerButtonPadding != mHeaderButtonPadding) {
            ...
            updateLayoutParams(mIconView, mTitleView, mMoveTaskButton, mDismissButton,mLockButton);
            if (mAppOverlayView != null) {
                updateLayoutParams(mAppIconView, mAppTitleView, null, mAppInfoView,mAppInfoView);
            }
        }
    }
    ...
    public void changeLockStatus(boolean change,Task t){
		if (t.key == null) {
			return;
		}
    	if (change){
    		RecentsTaskLoader.getInstance().setBaseIntentLock(t.key.baseIntent, !RecentsTaskLoader.getInstance().isBaseIntenInLock(t.key.baseIntent));
    	}
    	if (mCurrentPrimaryColorIsDark){//dark background,need to use light color
	        mLockButton.setImageResource(RecentsTaskLoader.initialize(mContext).isBaseIntenInLock(t.key.baseIntent)?
	        		R.drawable.app_lock : R.drawable.app_unlock);
    	}else{//light background,need to use dark color
	        mLockButton.setImageResource(RecentsTaskLoader.initialize(mContext).isBaseIntenInLock(t.key.baseIntent)?
	        		R.drawable.app_lock_grey : R.drawable.app_unlock_grey);    		
    	}

...
public class VolumePanel extends Handler {
    ...
    private static final int PLAY_SOUND_DELAY = 0;//AudioService.PLAY_SOUND_DELAY;
    ...
    public VolumePanel(Context context, ViewGroup parent, ZenModeController zenController) {
        ...
        //===modify by ty
        final boolean useMasterVolume = false;//res.getBoolean(R.bool.config_useMasterVolume);
        if (useMasterVolume) {
        ...
        if (parent == null) {
            ...
            lp.windowAnimations = com.android.systemui.R.style.ZenToastAnimations;
            lp.setTitle(TAG);
      ...
      //===modify by ty
        final boolean masterVolumeOnly = false;//res.getBoolean(R.bool.config_useMasterVolume);
    ...
    private void initZenModePanel() {
    	if(mZenPanel != null){//===modify by ty
    ...
    private void setLayoutDirection(int layoutDirection) {
        if(mPanel!=null)mPanel.setLayoutDirection(layoutDirection);
        updateStates();
    }
    ...
    private String getSuppressorCaption(ComponentName suppressor) {
        ...
        try {
            final ServiceInfo info = mPm.getServiceInfo(suppressor, 0);
            if (info != null) {
                final CharSequence seq = info.loadLabel(mPm);
                if (seq != null) {
    ...
    private void updateSliderEnabled(final StreamControl sc, boolean muted, boolean fixedVolume) {
        ...
        if (sc.streamType == STREAM_REMOTE_MUSIC) {
            ...
            sc.icon.setClickable(false);
                ...
                (sc.streamType != 
                //mAudioManager.getMasterStreamType()
    ...
    private void setZenPanelVisible(boolean visible) {
        ...
        if (visible) {
        	if (mZenPanel != null)mZenPanel.setHidden(false);
            resetTimeout();
        } else {
        	if (mZenPanel != null)mZenPanel.setHidden(true);
        }
        if (changing) {
            updateTimeoutDelay();
            resetTimeout();

...
public class VolumeDialog {
    ...
    private final long mExpandButtonAnimationDuration;
    ...
    /* yulong begin add */
    /* for change volume panel style, pengbin, 2016.1.13 */
    static boolean YULONG_VOLUMEPANEL_STYLE = true;
    private SparseBooleanArray mSpecialVolumeRow = new SparseBooleanArray();
    /* yulong end */
    private Drawable mYLDrawableVolumeCollapse;
    private Drawable mYLDrawableVolumeExpand;
    ...
    public VolumeDialog(Context context, int windowType, VolumeDialogController controller,
        ...
        mYLDrawableVolumeCollapse = context.getDrawable(R.drawable.yl_ic_volume_collapse_animation);
        mYLDrawableVolumeExpand = context.getDrawable(R.drawable.yl_ic_volume_expand_animation);
        ...
        /* yulong begin modify */
        /* for change volume panel style, pengbin, 2016.1.13 */
        if(YULONG_VOLUMEPANEL_STYLE){
            mActiveSliderTint = loadColorStateList(R.color.yl_volume_slider_active);
            mInactiveSliderTint = loadColorStateList(R.color.yl_volume_slider_inactive);
            mDialog.setContentView(R.layout.yl_volume_dialog);
        } else {
        ...
        if(YULONG_VOLUMEPANEL_STYLE){
            addRow(AudioManager.STREAM_RING,
                    R.drawable.yl_ic_volume_ring, R.drawable.yl_ic_volume_ring_silent, true);
            addRow(AudioManager.STREAM_MUSIC,
                    R.drawable.yl_ic_volume_music, R.drawable.yl_ic_volume_music_mute, true);
            addRow(AudioManager.STREAM_ALARM,
                    R.drawable.yl_ic_volume_alarm, R.drawable.yl_ic_volume_alarm_mute, false);
            addRow(AudioManager.STREAM_VOICE_CALL,
                    R.drawable.yl_ic_volume_call, R.drawable.yl_ic_volume_call, false);
            addRow(AudioManager.STREAM_BLUETOOTH_SCO,
                    R.drawable.yl_ic_volume_bt_sco, R.drawable.yl_ic_volume_bt_sco, false);
        } else {
            addRow(AudioManager.STREAM_RING,
...
//        mExpandButton.setTranslationX(x);
    	...
    	refreshPositionForExpendButton();
    }
    ...
    private VolumeRow initRow(final int stream, int iconRes, int iconMuteRes, boolean important) {
        ...
        /* for change volume panel style, pengbin, 2016.1.13 */
        if(YULONG_VOLUMEPANEL_STYLE){
            row.view = mDialog.getLayoutInflater().inflate(R.layout.yl_volume_dialog_row, null);
        } else {
    ...
    private void updateExpandButtonH() {
        ...
        int res = 0;
        if(YULONG_VOLUMEPANEL_STYLE) {
            res = mExpanded ? R.drawable.yl_ic_volume_collapse_animation
                    : R.drawable.yl_ic_volume_expand_animation;
        } else {
            res = mExpanded ? R.drawable.ic_volume_collapse_animation
...
//        mExpandButton.setImageResource(res);
        mExpandButton.setImageDrawable(mExpanded ? mYLDrawableVolumeCollapse : mYLDrawableVolumeExpand);
    ...
    private boolean isVisibleH(VolumeRow row, boolean isActive) {
        ...
        /* for change volume panel style, pengbin, 2016.1.13 */
        if(YULONG_VOLUMEPANEL_STYLE) {
            boolean isVisible = mExpanded && row.view.getVisibility() == View.VISIBLE
                    || (mExpanded && (row.important || isActive))
                    || !mExpanded && isActive;
            if (!isVisible) {
                if ((row.stream == AudioManager.STREAM_VOICE_CALL || row.stream == AudioManager.STREAM_BLUETOOTH_SCO) && mExpanded) {
                    if (mSpecialVolumeRow.get(row.stream) ) {
                        return true;
                    }
                }
            }
    ...
    private void onStateChangedH(State state) {
        ...
        if (mActiveStream != state.activeStream) {
            ...
            /* for change volume panel style, pengbin, 2016.1.13 */
            if(YULONG_VOLUMEPANEL_STYLE) {
                if (mActiveStream == AudioManager.STREAM_VOICE_CALL || mActiveStream == AudioManager.STREAM_BLUETOOTH_SCO) {
                    mSpecialVolumeRow.clear();
                    mSpecialVolumeRow.put(mActiveStream, true);
                }
            }
            /* yulong end */
            updateRowsH();
    ...
    private void updateVolumeRowH(VolumeRow row) {
        ...
        if(YULONG_VOLUMEPANEL_STYLE) {
            ...
            isRingStream = row.stream == AudioManager.STREAM_RING;
        }
        /* yulong end */
        ...
        if(YULONG_VOLUMEPANEL_STYLE){
            if(row.stream == AudioManager.STREAM_RING) {
                ...
                if(audioManager.getRingerMode()== AudioManager.RINGER_MODE_NORMAL) {
                    ...
                    iconRes = com.android.systemui.R.drawable.yl_ic_volume_ring_mute_vibrate;
                ...
                iconRes =
                        ...
                        : ss.routedToBluetooth ?
                                (ss.muted ? R.drawable.yl_ic_volume_media_bt_mute
                                        : R.drawable.yl_ic_volume_media_bt)
        ...
        if(YULONG_VOLUMEPANEL_STYLE){
            row.iconState =
                    ...
                    : (iconRes == R.drawable.yl_ic_volume_media_bt_mute || iconRes == row.iconMuteRes)
                            ? Events.ICON_STATE_MUTE
                    : (iconRes == R.drawable.yl_ic_volume_media_bt || iconRes == row.iconRes)
    ...
    private void updateVolumeRowHeaderVisibleH(VolumeRow row) {
        ...
        if(YULONG_VOLUMEPANEL_STYLE) {
            ...
            showHeaders = mShowHeaders || mExpanded && dynamic;
        }
        /* yulong end */
    ...
    private final VolumeDialogController.Callbacks mControllerCallbackH
        ...
        public void onConfigurationChanged() {
            updateWindowWidthH();
            /* yulong begin add */
            /* for change volume panel style, pengbin, 2015.12.10 */
            if(YULONG_VOLUMEPANEL_STYLE) {
                updateLocaleForVolumeRows();
            }
            /* yulong end */
            refreshPositionForExpendButton();
            mSpTexts.update();
    ...
    private void refreshPositionForExpendButton(){
       int x = 0;
       int y = 0;
        if(mContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT){
        	x = Utilities.dipToPixel(mContext, 304);
        	y = Utilities.dipToPixel(mContext, 29);
        }else if(mContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE){
        	x = Utilities.dipToPixel(mContext, 584);
        	y = Utilities.dipToPixel(mContext, 29);
        }
       mExpandButton.setTranslationX(x);
       mExpandButton.setTranslationY(y);
    ...
    /* yulong begin add */
    /* for change volume panel style, pengbin, 2016.1.13 */
    private void updateLocaleForVolumeRows() {
        mController.updateStreamTitles();
        for (VolumeRow row : mRows) {
            if (row != null) {
                if (row.ss != null) {
                 // update header text
                    String text = row.ss.name;
                    Util.setText(row.header, text);
                }
            }
        }

...
//        Log.d("", "init ccs="+mCountdownConditionSupported+" cd"+countdownDelta+
//        		" min="+minConditions+" max="+mMaxOptionalConditions);
            ...
            if (tag != null) {
                if (sameConditionId(tag.condition, mExitCondition)) {
                	try {
						...
						Log.d(TAG,"error: handleExitConditionChanged bind id fail");
					}
	...
	private static Condition parseExistingTimeCondition(Context context, Condition condition) {
		...
		if (span <= 0 || span > MAX_BUCKET_MINUTES * MINUTES_MS)
			return null;
		return ZenModeConfig.toTimeCondition(context, time, Math.round(span / (float) MINUTES_MS),
				ActivityManager.getCurrentUser(), false /* shortVersion */);
	}
    ...
    private void handleUpdateConditions() {
        ...
        try {
			...
			Log.d(TAG,"error: handleUpdateConditions bind id fail");
		}
    ...
    private void ensureSelection() {
        ...
        if (favoriteIndex == -1 || !mCountdownConditionSupported) {
            ...
            try {
				...
				Log.d(TAG,"error: ensureSelection bind id fail");
			}
    ...
    private void bind(final Condition condition, final View row) {
        ...
        if (tag.rb == null) {
        	RadioButton rb = (RadioButton) row.findViewById(android.R.id.checkbox);
        	if(rb == null){
        		Log.d(TAG,"error: bind id fail");
        		return;
        	}
            tag.rb = rb;
        }
    ...
    private void onClickTimeButton(View row, ConditionTag tag, boolean up) {
		...
		if (mBucketIndex == -1) {
			...
			if (newCondition == null) {
				mBucketIndex = DEFAULT_BUCKET_INDEX;
				newCondition = ZenModeConfig.toTimeCondition(mContext, MINUTE_BUCKETS[mBucketIndex],
        ...
        try {
			...
			Log.d(TAG,"error: onClickTimeButton bind id fail");
		}

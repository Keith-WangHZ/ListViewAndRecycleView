...
public class VolumeDialogController {
    ...
    private void onGetStateW() {
        try {
            for (int stream : STREAMS) {
                ...
                if (stream < mStreamTitles.length) {//YLH20160708:STREAMS.length == 10, mStreamTitles.length == 6
    ...
    private String getApplicationName(Context context, ComponentName component) {
        if (component == null) return null;
        //final PackageManager pm = context.getPackageManager();
        final String pkg = component.getPackageName();
        try {
            final ApplicationInfo ai = mPm.getApplicationInfo(pkg, 0);
            final String rt = Objects.toString(ai.loadLabel(mPm), "").trim();
            if (rt.length() > 0) {
                return rt;
            }
    ...
    private final class Receiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(AudioManager.VOLUME_CHANGED_ACTION)) {
                ...
                /* yulong begin add */
                /* for change volume panel style, pengbin, 2016.1.13 */
//                if(VolumeDialog.YULONG_VOLUMEPANEL_STYLE){
//                    if (!isAllowedUpdate(stream)) {
//                        Log.d(TAG, "onReceive VOLUME_CHANGED_ACTION we filter the stream=" + stream
//                                + " level=" + level + " oldLevel=" + oldLevel);
//                        return;
//                    }
//                }
    ...
    /* yulong begin add */
    /* for change volume panel style, pengbin, 2016.1.13 */
    void updateStreamTitles() {
        try {
            mStreamTitles = mContext.getResources().getStringArray(R.array.volume_stream_titles);
            for (int stream : STREAMS) {
                StreamState ss = mState.states.get(stream);
                if (ss != null && stream < mStreamTitles.length) {// YLH20160713:STREAMS.length == 10, mStreamTitles.length == 6
                    ss.name = mStreamTitles[stream];
                }
            }
        } catch (ArrayIndexOutOfBoundsException e) {
    ...
    private boolean isAllowedUpdate(int stream) {
        if (stream != AudioManager.STREAM_RING && stream != AudioManager.STREAM_MUSIC && stream != AudioManager.STREAM_ALARM && stream != AudioManager.STREAM_VOICE_CALL && stream != AudioManager.STREAM_BLUETOOTH_SCO) {
            return false;
        }
        return true;
    }

...
class KeyguardMessageArea extends TextView implements SecurityMessageDisplay {
	...
	static KeyguardMessageArea mMessageArea;
    ...
    private final Runnable mClearMessageRunnable = new Runnable() {
        @Override
        public void run() {
            mMessage = mMessageArea.mMessage;
            update();
        }
    };
    ...
    @Override
    public void setDefaultMessage(int resId, boolean showImmediately) {
        if (mMessageArea == null) {
        	return;
        }
		if (resId != 0) {
			mMessageArea.mDefaultMessage = getContext().getString(resId);
			if (showImmediately) {
				mMessageArea.mMessage = mMessageArea.getContext().getString(resId);
				securityMessageChanged(mMessageArea.mMessage);
			}
		}else {
            clearMessage();
        }
    ...
    public static SecurityMessageDisplay findSecurityMessageDisplay(View v) {
        mMessageArea = (KeyguardMessageArea) v.findViewById(

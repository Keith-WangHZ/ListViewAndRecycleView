...
//		return mStrongAuthTracker.isUnlockingWithFingerprintAllowed() && !hasFingerprintUnlockTimedOut(sCurrentUser);
	...
	public void stopListeningForFingerprint() {
		...
		if (/*mFingerprintRunningState == FINGERPRINT_STATE_RUNNING*/null != mFingerprintCancelSignal) {
			Log.v(TAG, "stopListeningForFingerprint() is cancel");
			mFingerprintCancelSignal.cancel();
	...
	public void makeSureStopFingerListenning(){
		if (null != mFingerprintCancelSignal) {
			Log.v(TAG, "make sure stopListeningForFingerprint() is cancel");
			mFingerprintCancelSignal.cancel();
			mFingerprintCancelSignal = null;
		}
	}
...
//			stopListeningForFingerprint();
	...
	public static boolean isCTSInstalled() {
		if (1 == SystemProperties.getInt("persist.yulong.ctstest", 0)) {
            return true;           
        }
		return false;
	}

...
public class CoolUINumPadKey extends ViewGroup {
    // list of "ABC", etc per digit, starting with '0'
    static String sKlondike[];

    private int mDigit = -1;
    private int mEditViewResId;
    private CoolUIEditView mEditView;
    ...
    private View.OnClickListener mListener = new View.OnClickListener() {
        ...
        public void onClick(View thisView) {
            if (mEditView == null && mEditViewResId > 0) {
                final View v = CoolUINumPadKey.this.getRootView().findViewById(mEditViewResId);
                if (v != null && v instanceof CoolUIEditView) {
                    mEditView = (CoolUIEditView) v;
                }
            }
            if (mEditView != null && mEditView.isEnabled()) {
            	mEditView.inputNums(String.valueOf(mDigit));
            }
            userActivity();
            doHapticKeyClick();
        }
    };
    private ObscureSpeechDelegate mObscureSpeechDelegate;
    public void userActivity() {
        mPM.userActivity(SystemClock.uptimeMillis(), false);
    }

    public CoolUINumPadKey(Context context) {
        this(context, null);
    }

    public CoolUINumPadKey(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CoolUINumPadKey(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            mEditViewResId = a.getResourceId(R.styleable.NumPadKey_textView, 0);
        } finally {
            a.recycle();
        }
        ...
        mObscureSpeechDelegate = new ObscureSpeechDelegate(context);
        setAccessibilityDelegate(mObscureSpeechDelegate);
...
//        ObscureSpeechDelegate.sAnnouncedHeadset = false;
          mObscureSpeechDelegate.setAnnouncedHeadset(false);
    }

...
//import android.util.BoostFramework;/// M modify for MTK
...
public abstract class PanelView extends FrameLayout {
	 ...
	 */
//	private BoostFramework mPerf = null;/// M modify for MTK
	private int mBoostParamVal[];

	/**
	 * Whether an instant expand request is currently pending and we are just
	 * waiting for layout.
	 */
	...
	/**
	 * Speed-up factor to be used when {@link #mFlingCollapseRunnable} runs the
	 * next time.
	 */
	...
	public PanelView(Context context, AttributeSet attrs) {
		...
		/// M modify for MTK
		/*boolean lIsPerfBoostEnabled = context.getResources()
				.getBoolean(com.android.internal.R.bool.config_enableCpuBoostForPanelViewFling);
		if (lIsPerfBoostEnabled) {
			mBoostParamVal = context.getResources()
					.getIntArray(com.android.internal.R.array.panelview_flingboost_param_value);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		// On expanding, single mouse click expands the panel instead of
		// dragging.
		...
		/*
		 * We capture touch events here and update the expand height here in
		 * case according to the users fingers. This also handles multi-touch.
		 *
		...
		case MotionEvent.ACTION_DOWN:
			...
			trackMovement(event);
			if (!mGestureWaitForTouchSlop || (mHeightAnimator != null && !mHintAnimationRunning) || mPeekPending
					|| mPeekAnimator != null) {
				cancelHeightAnimator();
				cancelPeek();
				mTouchSlopExceeded = (mHeightAnimator != null && !mHintAnimationRunning) || mPeekPending
						|| mPeekAnimator != null;
				onTrackingStarted();
			}
...
//				schedulePeek();
		...
		case MotionEvent.ACTION_POINTER_UP:
			...
			if (mTrackingPointer == upPointer) {
				...
				//inhibit two fingers's operation //===modify by ty
		...
		case MotionEvent.ACTION_MOVE:
			...
			// If the panel was collapsed when touching, we only need to check
			// for the
			// y-component of the gesture, as we have no conflicting horizontal
			// gesture.
	...
	/**
	 * @return whether the swiping direction is upwards and above a 45 degree
	 *         angle compared to the horizontal direction
	 */
  ...
  private void  onFlingToPrivateMode(float xv,float yv,float vv,float y){
	...
	private void endMotionEvent(MotionEvent event, float x, float y, boolean forceCancel, int bouncerPosition) {
		...
		if ((mTracking && mTouchSlopExceeded) || Math.abs(x - mInitialTouchX) > mTouchSlop
				|| Math.abs(y - mInitialTouchY) > mTouchSlop || event.getActionMasked() == MotionEvent.ACTION_CANCEL
				|| forceCancel) {
			//Log for fling by huazhi
			/*Log.d(TAG,
					"mTracking = " + mTracking + ", mTouchSlopExceeded = " + mTouchSlopExceeded + ", mTouchSlop = "
							+ mTouchSlop + ", Math.abs(y - mInitialTouchY) = " + Math.abs(y - mInitialTouchY)
							+ ", event.getActionMasked() = " + event.getActionMasked() + ", forceCancel = "
							+ forceCancel);*/
			float vel = 0f;
			float vectorVel = 0f;
			if (mVelocityTracker != null) {
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		/*
		 * If the user drags anywhere inside the panel we intercept it if the
		 * movement is upwards. This allows closing the shade from anywhere
		 * inside the panel.
		 *
		 * We only do this if the current content is scrolled to the bottom, i.e
		 * isScrolledToBottom() is true and therefore there is no conflicting
	...
	/**
	 * @return Whether a pair of coordinates are inside the visible view content
	 *         bounds.
	 */
	...
	/**
	 * @param x
	 *            the final x-coordinate when the finger was lifted
	 * @param y
	 *            the final y-coordinate when the finger was lifted
	...
	private boolean isFalseTouch(float x, float y) {
		...
		//turn off FalseTouch detection by huazhi
		/*if (mFalsingManager.isClassiferEnabled()) {
			Log.d("wanghuazhi:", "mFalsingManager.isFalseTouch() = " + mFalsingManager.isFalseTouch());
	...
	protected void flingToHeight(float vel, boolean expand, float target, float collapseSpeedUpFactor,
		...
		final boolean clearAllExpandHack = expand && fullyExpandedClearAllVisible()
				&& mExpandedHeight < getMaxPanelHeight() - getClearAllHeight() && !isClearAllVisible();
		...
		if (expand) {
			...
			if (vel == 0) {
				animator.setDuration(
						(long) (animator.getDuration() * getCannedFlingDurationFactor() / collapseSpeedUpFactor));
			}
		}
		/// M modify for MTK
//		if (mPerf != null) {
...
//				}
	...
	protected void requestPanelHeightUpdate() {
		...
		if ((!mTracking || isTrackingBlocked()) && mHeightAnimator == null && !isFullyCollapsed()
				&& currentMaxPanelHeight != mExpandedHeight && !mPeekPending && mPeekAnimator == null
				&& !mPeekTouching) {
	...
	/**
	 * @return true if the panel tracking should be temporarily blocked; this is
	 *         used when a conflicting gesture (opening QS) is happening
	 */
	...
	/**
	 * This returns the maximum height of the panel. Children should override
	 * this if their desired height is not the full height.
	 *
	...
	public void collapse(boolean delayed, float speedUpFactor) {
		...
		if (mPeekPending || mPeekAnimator != null) {
			...
			if (mPeekPending) {
				...
				// We know that the whole gesture is just a peek triggered by a
				// simple click, so
				// better start it now.
				removeCallbacks(mPeekRunnable);
				mPeekRunnable.run();
			...
			// Set after notifyExpandingStarted, as notifyExpandingStarted
			// resets the closing state.
			mClosing = true;
			if (delayed) {
	...
	private final Runnable mFlingCollapseRunnable = new Runnable() {
		@Override
		public void run() {
			fling(0, false /* expand */, mNextCollapseSpeedUpFactor, false /* expandBecauseOfFalsing */);
		}
	};
	...
	public void cancelPeek() {
		...
		if (cancelled) {
			// When peeking, we already tell mBar that we expanded ourselves.
			// Make sure that we also
	...
	public void expand(final boolean animate) {
		...
		cancelPeek();
		if (mTracking) {
			onTrackingStopped(true /* expands */); // The panel is expanded
													// after this call.
		}
		if (mExpanding) {
			notifyExpandingFinished();
		...
		// Wait for window manager to pickup the change, so we know the maximum
		// height of the panel
		// then.
	...
	protected void startUnlockHintAnimation() {
		...
		// We don't need to hint the user if an animation is already running or
	...
	protected void notifyBarPanelExpansionChanged() {
		mBar.panelExpansionChanged(mExpandedFraction, mExpandedFraction > 0f || mPeekPending || mPeekAnimator != null
				|| mInstantExpanding || isPanelVisibleBecauseOfHeadsUp() || mTracking || mHeightAnimator != null);
	}

	protected abstract boolean isPanelVisibleBecauseOfHeadsUp();

	/**
	 * Gets called when the user performs a click anywhere in the empty area of
	 * the panel.
	 *
	 * @return whether the panel will be expanded after the action performed by
	 *         this method
	 */
	...
	public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
		pw.println(String.format(
				"[PanelView(%s): expandedHeight=%f maxPanelHeight=%d closing=%s"
						+ " tracking=%s justPeeked=%s peekAnim=%s%s timeAnim=%s%s touchDisabled=%s" + "]",
				this.getClass().getSimpleName(), getExpandedHeight(), getMaxPanelHeight(), mClosing ? "T" : "f",
				mTracking ? "T" : "f", mJustPeeked ? "T" : "f", mPeekAnimator,
				((mPeekAnimator != null && mPeekAnimator.isStarted()) ? " (started)" : ""), mHeightAnimator,
	...
	/**
	 * @return whether "Clear all" button will be visible when the panel is
	 *         fully expanded
	 */

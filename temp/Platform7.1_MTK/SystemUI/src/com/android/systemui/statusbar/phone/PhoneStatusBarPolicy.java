...
public class PhoneStatusBarPolicy implements Callback, RotationLockController.RotationLockControllerCallback, DataSaverController.Listener {
    ...
    /// M modify for MTK
    private static final String SLOT_IMS = "ims";
    public static boolean mImsEnabled = false;
...
//            mIconController.setIconVisibility(mSlotHotspot, enabled);
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (action.equals(ImsManager.ACTION_IMS_STATE_CHANGED)) {/// M modify for MTK
                updateIms(intent);
            }
        }
    };
  ...
  /// M: Add for volte @{
    ...
    private boolean isImsOverVoice(Intent intent) {
        return getImsEnableCap(intent) == ImsConfig.FeatureConstants.FEATURE_TYPE_VOICE_OVER_LTE;
    }

    /// @}
    //yulong begin: add for ImsManagerService in SystemUI,shenyupeng,2015.12.09
    private final void updateIms(Intent intent) {
    	
//        int mImsServiceState = intent.getIntExtra(ImsManager.EXTRA_IMS_SERVICE_STATE_KEY/*android:serviceState*/, 0);
//        Log.d(TAG,"mImsServiceState = " + mImsServiceState);
        mService.setIcon(SLOT_IMS, R.drawable.stat_sys_hd_voice_on, 0, null);
        mImsEnabled =false;
        
        int imsRegState = intent.getIntExtra(ImsManager.EXTRA_IMS_REG_STATE_KEY,
                ServiceState.STATE_OUT_OF_SERVICE);
        Log.d(TAG,"mImsServiceState = " + imsRegState);
        if (isImsOverVoice(intent) && //isLteNetWork() &&
                imsRegState == ServiceState.STATE_IN_SERVICE) {
        	mImsEnabled = true;
        }
//        if (mImsServiceState == 1){
//            mImsEnabled = true;
//        }
        mService.setIconVisibility(SLOT_IMS, mImsEnabled);
    }
    //yulong end

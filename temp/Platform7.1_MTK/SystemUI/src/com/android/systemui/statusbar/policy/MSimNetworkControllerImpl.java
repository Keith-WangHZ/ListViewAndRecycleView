...
public class MSimNetworkControllerImpl extends NetworkControllerImpl {
    ...
    protected int mDataUsingPhoneId = -1;// 浣跨敤鐨凱honeId
    ...
    int PHONE_ID3 = PhoneConstants.SIM_ID_3;/// M modify for MTK
    ...
    public interface YulongSignalCluster {
        ...
        // 璁剧疆淇″彿鍥炬爣鍜岀被鍨嬪浘鏍噋honeId 0,1,3鍒嗗埆琛ㄧず鍕�1锛屽崱2锛屽崱3...
        ...
        //if (subInfoList != null) {
            ...
            for (int i=0; i < mPhoneCount; i++) {
                ...
                if (subIdtemp != null) {
                    ...
                    mSubIdPhoneIdMap.put(subId, i);/// M modify for MTK
...
//                        mSubIdPhoneIdMap.put(subId, i);
                        ...
                        mPhone.listen(mMSimPhoneStateListener[i],
                                        ...
                                        | PhoneStateListener.LISTEN_PRECISE_DATA_CONNECTION_STATE);/// M modify for MTK
                    } else {
    ...
    public void refreshSimCardVisible(SignalClusterViewYuLong cluster){
        ...
        if(YulongConfig.getDefault().getNetworkType().endsWith("WG")){
        	...
        	//鐢靛晢椤圭洰鍦ㄦ湁涠夸釜鍗″惎鍔ㄧ殑鏃跺�欒鑷姩闅愯棌鏃犲崱鍥炬爣
...
//        } 
    ...
    private class LocalPhoneStateListener extends  PhoneStateListener{
    	...
    	//鍏ㄩ儴灏佽鍒拌繖涓洃鍚被涓紝鍥犱负杩欎簺鏁版嵁鏄繀瀹氬拰鐩戝惉缁戝畾鐦�
        // telephony 
...
//        private LocalPhoneStateListener mMSimPhoneStateListener;
        ...
        private HashSet<String> mPreciseDataConnectedState = new HashSet<String>();/// M modify for MTK
         ...
         * begin add by wangsheng@yulong.com 20151124
         */
      @Override
      public void onPreciseDataConnectionStateChanged(PreciseDataConnectionState state) {
            int phoneId = mSlotId;
            Log.d(TAG, "phoneId=" + phoneId + ";onPreciseDataConnectionStateChanged: state = " + state.toString() + "; this=" + this);
            LogHelper.sd(TAG, "phoneId=" + phoneId + ";onPreciseDataConnectionStateChanged: state = " + state.toString());
            ...
            if (dataState == TelephonyManager.DATA_CONNECTED) {
                if (!mPreciseDataConnectedState.contains(apnType)) {
                    ...
                    Log.d(TAG, "phoneId=" + phoneId + ";onPreciseDataConnectionStateChanged: put apnType: " + apnType +  ", dataState: " + dataState + " into mPreciseDataConnectedState");
                    LogHelper.sd(TAG, "phoneId=" + phoneId + ";onPreciseDataConnectionStateChanged: put apnType: " + apnType +  ", dataState: " + dataState + " into mPreciseDataConnectedState");
                    updateIconSet(phoneId);
                    updateTelephonySignalStrength(phoneId);
                    updateDataNetType(phoneId);
                    updateDataIcon(phoneId);
            ...
            } else if (mPreciseDataConnectedState.contains(apnType)) {
                    mPreciseDataConnectedState.remove(apnType);
                    Log.d(TAG, "phoneId=" + phoneId + ";onPreciseDataConnectionStateChanged: remove apnType: " + apnType + ", dataState: " + dataState + " from mPreciseDataConnectedState");
                    LogHelper.sd(TAG, "phoneId=" + phoneId + ";onPreciseDataConnectionStateChanged: remove apnType: " + apnType + ", dataState: " + dataState + " from mPreciseDataConnectedState");
                    updateIconSet(phoneId);
                    updateTelephonySignalStrength(phoneId);
                    updateDataNetType(phoneId);
                    updateDataIcon(phoneId);
...
//		}.execute();
    ...
    private void updateIconSet(int phoneId) {
        ...
        if ( TelephonyManager.getNetworkClass(dataNetworkType) == TelephonyManager.getNetworkClass(voiceNetworkType)
        		...
        		|| !isChinaTelecomSIM()){
        	mMSimPhoneStateListener[phoneId].mSignalSingleMode = true;//鍙樉绀哄崟鎸�
...
//        }
        ...
        boolean mShowDataActivityIcon = isShowDataActivityIcon(phoneId);/// M modify for MTK

        LogHelper.sd(TAG,"updateDataIcon  when mDataNetType =" + mMSimPhoneStateListener[phoneId].mDataNetType);
        LogHelper.sd(TAG,"updateDataIcon  mShowDataActivityIcon =" + mShowDataActivityIcon);
        LogHelper.sd(TAG,"updateDataIcon  when mDataState = =" + mMSimPhoneStateListener[phoneId].mDataState);

        if (mMSimPhoneStateListener[phoneId].mDataNetType == TelephonyManager.NETWORK_TYPE_UNKNOWN
        		 || (!mShowDataActivityIcon)) {/// M modify for MTK
            visible = false;
        } else {
            ...
            if (mMSimPhoneStateListener[phoneId].mMSimState == IccCardConstants.State.READY ||
                ...
                if (mMSimPhoneStateListener[phoneId].mDataState == TelephonyManager.DATA_CONNECTED) {
                    ...
                    } else if (phoneId == PHONE_ID3) {/// M modify for MTK
                        mDataUsingPhoneIconId = R.drawable.stat_sys_signal_type_card3;
	                }                    
                } else {
                    iconId = 0;
                    visible = false;
    ...
    /// M modify for MTK begin
    /**
     * Check if show data activity icon.
     * @return if show data activity icon.
     * begin add by wangsheng@yulong.com 20151124
     */
    private boolean isShowDataActivityIcon(int phoneId) {
        if (DEBUG) {
            for (String apn : mMSimPhoneStateListener[phoneId].mPreciseDataConnectedState) {
                LogHelper.sd(TAG, "phoneId=" + phoneId + "; isShowDataActivityIcon(),current connected apn type: " + apn);
            }
        }
        if (mMSimPhoneStateListener[phoneId].mPreciseDataConnectedState.size() == 0) {
            boolean isMobileDataEnabled = CPDataConnSettingUtils.getMobileDataEnabled(mContext);
            LogHelper.sd(TAG, "phoneId=" + phoneId + "; mPreciseDataConnectedState.size() = 0, isShowDataActivityIcon(), return:" + isMobileDataEnabled);
            return isMobileDataEnabled;
        } else if (mMSimPhoneStateListener[phoneId].mPreciseDataConnectedState.size() == 1) {
            if (mMSimPhoneStateListener[phoneId].mPreciseDataConnectedState.contains(PhoneConstants.APN_TYPE_IMS) ||
            		mMSimPhoneStateListener[phoneId].mPreciseDataConnectedState.contains(PhoneConstants.APN_TYPE_EMERGENCY)) {
                LogHelper.sd(TAG, "phoneId=" + phoneId + "; mPreciseDataConnectedState.size() = 1, isShowDataActivityIcon(), return false");
                return false;
            }
        } else if (mMSimPhoneStateListener[phoneId].mPreciseDataConnectedState.size() == 2) {
            if (mMSimPhoneStateListener[phoneId].mPreciseDataConnectedState.contains(PhoneConstants.APN_TYPE_IMS) &&
            		mMSimPhoneStateListener[phoneId].mPreciseDataConnectedState.contains(PhoneConstants.APN_TYPE_EMERGENCY)) {
                LogHelper.sd(TAG, "phoneId=" + phoneId + "; mPreciseDataConnectedState.size() = 2, isShowDataActivityIcon(), return false");
                return false;
            }
        }
        LogHelper.sd(TAG, "phoneId=" + phoneId + "; isShowDataActivityIcon(), return true, " +
                "mPreciseDataConnectedState.size(): " + mMSimPhoneStateListener[phoneId].mPreciseDataConnectedState.size());
        return true;
    }
    /// M modify for MTK end
    ...
    protected void calculatePhoneSignalIcon(int phoneId, int iconLvl, int slaveIconLvl) {
        ...
        if (iconLvl == -1 && slaveIconLvl == -1){
        	...
        	if(networktype.equalsIgnoreCase("CG")){
        		/// M modify for MTK YLH20160701: for CTCC
        		mMSimPhoneStateListener[phoneId].mPhoneSignalTypeIconId = R.drawable.stat_sys_signal_0;
        		icon = 0;        		
        ...
        } else if ( mMSimPhoneStateListener[phoneId].mSignalSingleMode || slaveIconLvl == -1 || iconLvl == -1){ //  鍗曚俊鍗�
        	if (isSEDRegion()){
	        	...
	        	String networkType = YulongConfig.getDefault().getNetworkType();
	          	if(YulongConfig.getDefault().getNetworkType().endsWith("WG")
	        		...
	        		icon = R.drawable.stat_sys_signal_type_hp;
	          			...
	          			|| networkType.endsWith("AGG") || networkType.endsWith("ACG")) {/// M modify for MTK
...
//	        	nLevel = signal.getGsmLevel();
	        	nLevel = signal.getLevel();/// M modify for MTK
...
//	        	nLevel = signal.getTdScdmaLevel();//signal.getTdscdmaLevel();
	        	nLevel = signal.getCdmaLevel();/// M modfify for MTK
